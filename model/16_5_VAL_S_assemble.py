...
from wdt_fmt import fmt_pct as pct, out_dir, ensure_dir
from wdt_md import md_table
import wdt_analytics as _A
from wdt_analytics import init, make_p, run_sim_p, c1, n_crossing

_OUT = out_dir('VAL_S')
OUT_FILE = str(_OUT / 'VAL_S_Appendix_Tables.md')

# These are accessed via _A.CANON_* after init() to avoid the
# from-import rebinding problem (from X import Y captures None at
# import time; _A.Y reads the current module attribute at call time).
# Convenience shorthands used throughout section functions:
def _canon_tau0():  return _A.CANON_TAU0
def _canon_taum():  return _A.CANON_TAUM
def _canon_k():     return _A.CANON_K
def _canon_n():     return _A.CANON_N
def _canon_v0():    return _A.CANON_V0
def _canon_g():     return _A.CANON_G
def _canon_wmin():  return _A.CANON_WMIN

# Grid constants — accessed via _A module reference throughout
# (not imported directly; would capture None at import time)

# Grid constants populated by main() after init()
N_VALS            = None
TAU0_N_GRID_TAU0  = None
TAU0_N_GRID_NCEIL = None
K_V0_K_GRID       = None
K_V0_V0_GRID      = None
...
def section_a1_tau0():
    ...
    lines.append(
        ...
        f'$\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, $k$ = {_A.CANON_K}, N = {_A.CANON_N}, $V_0$ = £{_A.CANON_V0:.0f}m.  '
        '$\\alpha$ = 1.0 row is zero by construction.'
    )
    lines.append('')

    for tau_0 in _A.TAU0_VALS:
        canon_mark = '  *(canonical)*' if tau_0 == _A.CANON_TAU0 else ''
        lines.append(f'### B.1.{_A.TAU0_VALS.index(tau_0)+1}  $\\tau_0$ = {tau_0*100:.0f}%{canon_mark}')
        lines.append('')
        p = make_p(tau_0=tau_0)
        headers = ['$\\alpha$ \\ $g$'] + _A.G_LABELS
        rows = []
        for alpha in _A.ALPHA_VALS:
            row = [f'**{alpha}**'] + [pct(c1(p, alpha, g_val)) for g_val in _A.G_VALS]
            rows.append(row)
...
def section_a2_taum():
    ...
    lines.append(
        f'**Metric:** (Net($\\alpha$) − Net(1) / TW($\\alpha$)  ·  '
        f'$\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, $k$ = {_A.CANON_K}, N = {_A.CANON_N}, $V_0$ = £{_A.CANON_V0:.0f}m.'
    )
    lines.append('')

    for tau_m in _A.TAUM_VALS:
        canon_mark = '  *(canonical)*' if tau_m == _A.CANON_TAUM else ''
        lines.append(f'### B.2.{_A.TAUM_VALS.index(tau_m)+1}  $\\tau_m$ = {tau_m*100:.0f}%{canon_mark}')
        lines.append('')
...
def section_a3_k():
    ...
    lines.append(
        f'**Metric:** (Net($\\alpha$) − Net(1) / TW($\\alpha$)  ·  '
        f'$\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, $\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, N = {_A.CANON_N}, $V_0$ = £{_A.CANON_V0:.0f}m.'
    )
    lines.append('')

    for k_val in _A.K_VALS:
        canon_mark = '  *(canonical)*' if k_val == _A.CANON_K else ''
        lines.append(f'### B.3.{_A.K_VALS.index(k_val)+1}  $k$ = {k_val}{canon_mark}')
        lines.append('')
        p = make_p(k=k_val)
        headers = ['$\\alpha$ \\ $g$'] + _A.G_LABELS
        rows = []
...
def section_a4_n():
    ...
    lines.append(
        ...
        f'$\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, $\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, $k$ = {_A.CANON_K}, '
        f'$V_0$ = £{_A.CANON_V0:.0f}m, $g$ = {_A.CANON_G*100:.2f}% throughout.  '
        '$\\alpha$ = 1.0 row is zero by construction.'
    )
    lines.append('')
    ...
    for n in N_VALS:
        ...
        lines.append('')
        headers = ['$\\alpha$', f'C.1 at $g$ = {_A.CANON_G*100:.2f}%', 'TW (£m)', 'Net (£m)', 'Eff rate']
        rows = []
        for alpha in _A.ALPHA_VALS:
...
def section_a5_v0():
    ...
    lines.append(
        ...
        f'$\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, $\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, $k$ = {_A.CANON_K}, N = {_A.CANON_N}.'
    )
    lines.append('')

    for v0 in _A.V0_VALS:
        canon_mark = '  *(canonical)*' if v0 == _A.CANON_V0 else ''
        lines.append(f'### B.5.{_A.V0_VALS.index(v0)+1}  $V_0$ = £{v0:.0f}m{canon_mark}')
        lines.append('')
        p = make_p(V0_m=v0)
...
def section_a6_tau0_n_surface():
    ...
    lines.append('')
    lines.append(
        f'**Metric:** First N at which Net($\\alpha$=2.0) > Net($\\alpha$=1.0) at $g$ = {_A.CANON_G*100:.1f}%.  '
        f'$\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, $k$ = {_A.CANON_K}, $V_0$ = £{_A.CANON_V0:.0f}m.  '
    ...
    lines.append('')
    lines.append(
        f'*Canonical cell: $\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, N ceiling = {_A.CANON_N}.*'
    )
    lines.append('')
...
def section_a7_k_v0_surface():
    ...
    lines.append('')
    lines.append(
        f'**Metric:** (Net(1.8) − Net(1.0) / TW(1.8) at $g$ = {_A.CANON_G*100:.1f}%, N = {_A.CANON_N}.  '
    ...
    for k_val in K_V0_K_GRID:
        ...
        for v0 in K_V0_V0_GRID:
            ...
            row.append(pct(c1(p, alpha=1.8, g=_A.CANON_G)))
        rows.append(row)
    lines.append(md_table(headers, rows))
    lines.append('')
    lines.append(
        f'*Canonical cell: $k$ = {_A.CANON_K}, $V_0$ = £{_A.CANON_V0:.0f}m.*'
    )
    lines.append('')
...
def section_a8_wmin():
    ...
    lines.append(
        f'**Metric:** (Net($\\alpha$) − Net(1) / TW($\\alpha$))  ·  '
        f'$\\tau_0$ = {_A.CANON_TAU0*100:.0f}%, $\\tau_m$ = {_A.CANON_TAUM*100:.0f}%, $k$ = {_A.CANON_K}, '
        f'N = {_A.CANON_N}, $V_0$ = £{_A.CANON_V0:.0f}m, $g$ = {_A.CANON_G*100:.2f}% throughout.  '
    ...
    for w_min in _A.WMIN_VALS:
        canon_mark = '  *(canonical)*' if w_min == _A.CANON_WMIN else ''
        lines.append(f'### B.8.{_A.WMIN_VALS.index(w_min)+1}  $W_{{min}}$ = £{w_min:.0f}m{canon_mark}')
        lines.append('')
...
FIGURE_REGISTRY = []   # populated by _build_figure_registry() in main()


def _build_figure_registry():
    """Build the figure registry after CANON_* constants have been set by init()."""
    return [
    {
        'ref':   'S2.1a',
        ...
        'params': f'$\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m; $\\tau_0$ swept across panels',
        'val_a': 'C.1',
    },
    {
        'ref':   'S2.1b',
        ...
        'params': f'$\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, $V_0$=£{_A.CANON_V0:.0f}m, $g$={_A.CANON_G*100:.1f}%',
        'val_a': 'C.7, B.5.6',
    },
    {
        'ref':   'S2.1c',
        ...
        'params': f'$\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}, $g$={_A.CANON_G*100:.1f}%',
        'val_a': 'B.6',
    },
    {
        'ref':   'S2.2a',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m',
        'val_a': 'C.1',
    },
    {
        'ref':   'S2.2b',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}; plateau evaluated at $g$ = 18–40%',
        'val_a': 'C.9, B.5.4',
    },
    {
        'ref':   'S2.2c',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, k={_A.CANON_K}, $V_0$=£{_A.CANON_V0:.0f}m, $g$={_A.CANON_G*100:.1f}%',
        'val_a': 'C.7, B.5.4',
    },
    {
        'ref':   'S2.3a',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, $W_{{min}}$=£{_A.CANON_WMIN:.0f}m',
    ...
    },
    {
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m',
        'val_a': 'C.1, C.5',
    },
    {
        'ref':   'S2.3c',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, N={_A.CANON_N}, $g$={_A.CANON_G*100:.1f}%',
        'val_a': 'C.1, B.5.2',
    },
    {
        'ref':   'S3.1a',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, $V_0$=£{_A.CANON_V0:.0f}m, $g$={_A.CANON_G*100:.1f}%',
    ...
    },
    {
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, $V_0$=£{_A.CANON_V0:.0f}m',
    ...
    },
    {
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, $g$={_A.CANON_G*100:.1f}%',
        'val_a': 'B.6',
    },
    {
        'ref':   'S3.2a',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}, $g$={_A.CANON_G*100:.1f}%',
    ...
    },
    {
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}',
        'val_a': 'B.3.1, Fig 5.1 in VAL',
    },
    {
        'ref':   'S3.2c',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}',
        'val_a': 'C.1',
    },
    {
        'ref':   'S2.4a',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}; $W_{{min}}$ swept',
        'val_a': 'B.3.1, Fig 5.1 in VAL',
    },
    {
        'ref':   'S2.4b',
        ...
        'params': f'$\\tau_0$={_A.CANON_TAU0*100:.0f}%, $\\tau_m$={_A.CANON_TAUM*100:.0f}%, k={_A.CANON_K}, N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m',
        'val_a': 'C.1',
    },
    {
        'ref':   'S2.4c',
    ...
    },
    {
        ...
        'params': f'N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m, $g$={_A.CANON_G*100:.1f}%; all $\\tau_0$/$\\tau_m$/k/$W_{{min}}$ variants shown together',
...
def main():
    ...
    # Initialise analytics constants and surface grids
    global N_VALS, TAU0_N_GRID_TAU0, TAU0_N_GRID_NCEIL, K_V0_K_GRID, K_V0_V0_GRID
    p = load_params()
    init(p)
    sw = p['sweep']
    N_VALS            = _A.N_PANEL_VALS
    TAU0_N_GRID_TAU0  = sw['tau0_n_surface_tau0']
    TAU0_N_GRID_NCEIL = sw['tau0_n_surface_nceil']
    K_V0_K_GRID       = sw['k_v0_surface_k']
    K_V0_V0_GRID      = sw['k_v0_surface_v0']

    global FIGURE_REGISTRY
    FIGURE_REGISTRY = _build_figure_registry()
    ...
    lines.append(
        ...
        f'k={_A.CANON_K}, $W_{{min}}$=£{_A.CANON_WMIN:.0f}m, N={_A.CANON_N}, $V_0$=£{_A.CANON_V0:.0f}m, $g$={_A.CANON_G*100:.2f}%'
    )
    lines.append('')
    lines.append(
    ...
    md = '\n'.join(lines)
    out_path = _OUT / 'VAL_S_Appendix_Tables.md'
