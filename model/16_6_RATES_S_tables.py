...
from wdt_fmt import fmt_pct1 as fmt_pct, fmt_f, baseline_marker, dist_row, out_dir
...
def _md_param_section(sweep_results, param_label, baseline_v, baseline_name,
    ...
    Summary table columns (v8):
      Value | Success% | SSMcov{HW} | TCMcov{HW} | SSMcov50 | LRR fail n
      | LRR fill yr | SRR fill med | LRR surplus (min/med)

    where HW = HEADLINE_WINDOW.  The headline window is set in wdt_analytics.py.
    The 50yr column shows long-run trajectory alongside the headline.
    LRR fail n = count of start years where lrr_failure_year is not None.

    Worst-case 2006 table columns (v8):
      Value | SSMcov{HW} | TCMcov{HW} | LRR fill yr | LRR surplus £b | LRR failure yr
    """
    lines = []
    A = lines.append

    hw = HEADLINE_WINDOW   # local alias for brevity in f-strings
    ...
    A('')
    A(f'| Value | Success% | SSMcov{hw} (min/med/mean/max) | TCMcov{hw} (min/med/mean/max) | '
      f'SSMcov50 (min/med) | LRR fail n | '
      f'LRR fill yr (min/med/mean/max) | SRR fill yr (med) | LRR surplus £b (min/med) |')
    A('|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|')
    ...
    for r in sweep_results:
        ...
        A(f'| {value_fmt(v)}{mark} '
          ...
          f'| {fmt_f(s["lrr_surplus"]["min"], ".0f")} / {fmt_f(s["lrr_surplus"]["median"], ".0f")} |')

    A('')
    A(f'*◄ = Balanced baseline value. '
      f'SSMcov{hw}/TCMcov{hw}: Step-5 coverage fraction averaged over {hw} post-fill years '
      f'(headline window; set HEADLINE_WINDOW in wdt_analytics.py to change). '
    ...
    A('')
    A(f'| Value | SSMcov{hw} | TCMcov{hw} | LRR fill yr | LRR surplus £b | LRR failure yr |')
    A('|:---:|:---:|:---:|:---:|:---:|:---:|')
    ...
    for r in sweep_results:
        ...
        A(f'| {value_fmt(v)}{mark} | {ssm} | {tcm} | {lrr} | {sur} | {fail} |')
...
def write_report(sweep_tau_0, sweep_tau_m, sweep_k, sweep_wmin,
    ...
    A(f'**Model version:** v8 (rates_model.py / wdt_core.py)  ')
    A(f'**Headline coverage window:** {HEADLINE_WINDOW} years '
      f'(SSMcov{HEADLINE_WINDOW}/TCMcov{HEADLINE_WINDOW} columns; '
      f'change HEADLINE_WINDOW in wdt_analytics.py)  ')
    ...
    A('### B.1.3 Metrics')
    A('')
    A('**Success (v8):** LRR fills within the 71-year modelling window AND '
      'the LRR buffer never hits zero (lrr_failure_year is None). '
      'At Balanced parameters, success rate is 100% across all 73 start years.')
    A('')
    A(f'**SSMcov{HEADLINE_WINDOW} / TCMcov{HEADLINE_WINDOW} (headline window):** '
      f'Average Step-5 coverage fraction (labour-tax-relief surplus / annual expenditure) '
      f'over the first {HEADLINE_WINDOW} post-fill years. '
      'SSM applies uniform historical returns (correlated-shock, worst-case floor); '
      'TCM applies heterogeneous tier differentials (persistent-heterogeneity ceiling). '
      'Zero in any year where the LRR or SRR balance hits zero drags the average down. '
      f'The headline window is {HEADLINE_WINDOW} years; change HEADLINE_WINDOW in '
      'wdt_analytics.py to switch all tables and charts simultaneously.')
    A('')
    A('**SSMcov50:** Same metric averaged over 50 post-fill years. '
      'Shows long-run trajectory: rising values indicate WDT revenue compounds faster '
      'than expenditure; falling values indicate the coverage promise weakens with time.')
    A('')
    A('**LRR fail n:** Count of the 73 historical start years where the LRR balance '
      'reaches zero within the 71-year modelling window (lrr_failure_year is not None). '
      'At Balanced parameters this is 0. Non-zero values under stressed parameters '
      'indicate the post-fill buffer is insufficient for some historical return sequences.')
    A('')
    A('**LRR failure year:** First year the LRR balance hits zero post-fill. '
      'The LRR failing means the political buffer is exhausted; any further SRR deficit '
      'has no backstop. The SRR failure year (when the refund guarantee itself breaks) '
      'follows later.')
    A('')
    A('**LRR fill year:** First year the LRR balance reaches the floor target. '
    ...
    A('# C. Reading Notes')
    A('')
    A('**Coverage window direction.** SSMcov and TCMcov move together when a parameter '
      'raises or lowers revenue across the distribution. The SSM–TCM gap measures '
      'sensitivity to persistent growth heterogeneity; a wide gap means higher-tier '
      'taxpayers contribute disproportionately more than the correlated-shock assumption. '
      'The 50yr window is larger than the headline window because WDT revenue compounds '
      'on a growing wealth base; falling 50yr values relative to the headline signal '
      'that the coverage promise weakens in the long run for that calibration.')
    A('')
    A('**LRR failure vs. non-fill.** Two distinct failure modes: (1) LRR never fills '
      '(success_rate < 100%) — the mechanism does not reach Phase Two at all for some '
      'start years; (2) LRR fails post-fill (LRR fail n > 0) — Phase Two begins but the '
      'post-fill buffer is later exhausted. Both are solvency constraints, but at '
      'Balanced parameters only stressed SWF sizing parameters (high srr_ratio or '
      'lrr_years) or very low rate parameters produce non-zero LRR fail n.')
    A('')
