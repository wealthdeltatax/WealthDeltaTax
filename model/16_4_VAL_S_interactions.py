...
from wdt_style import apply_style_nogrid, save_fig
import wdt_analytics as _A
from wdt_analytics import (init, make_p, run_sim_p, c1,
                            n_crossing, tolerant_zone_width, understater_plateau)

_OUT = out_dir('VAL_S')

# §4 figures are heatmaps — no grid; alias for internal call sites
set_style = apply_style_nogrid
...
# Surface grids — populated in main() after init()
_SW_TAU0_N_TAU0  = None
_SW_TAU0_N_NCEIL = None
_SW_K_V0_K       = None
_SW_K_V0_V0      = None
...
def fig_tau0_n_surface():
    ...
    tau0_grid   = _SW_TAU0_N_TAU0
    n_ceil_grid = _SW_TAU0_N_NCEIL
...
def fig_k_v0_surface():
    ...
    for i, k_val in enumerate(k_grid):
        for j, v0 in enumerate(v0_grid):
            ...
            surface[i, j] = c1(p, alpha=1.8, g=_A.CANON_G) * 100
    ...
    # Mark canonical cell
    can_k_idx  = k_grid.index(_A.CANON_K)
    can_v0_idx = v0_grid.index(_A.CANON_V0)
...
def fig_calibration_summary():
    ...
    fig.suptitle(
        ...
        'Bold borders = canonical values  ·  N = {}, V₀ = £{}m, g = {:.1f}%'.format(
            _A.CANON_N, int(_A.CANON_V0), _A.CANON_G * 100),
        fontsize=10
    )
    plt.tight_layout()
...
def main():
    ...
    global _SW_TAU0_N_TAU0, _SW_TAU0_N_NCEIL, _SW_K_V0_K, _SW_K_V0_V0
    _SW_TAU0_N_TAU0  = sw['tau0_n_surface_tau0']
    _SW_TAU0_N_NCEIL = sw['tau0_n_surface_nceil']
    _SW_K_V0_K       = sw['k_v0_surface_k']
    _SW_K_V0_V0      = sw['k_v0_surface_v0']
