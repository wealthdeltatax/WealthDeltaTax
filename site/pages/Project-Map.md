...
**Demand expansion and economic reallocation.** §4.5 sets out the demand expansion hypothesis explicitly as speculative and Phase One-dependent: if households spend a substantial proportion of additional purchasing power, the resulting demand increase should alter the composition of economic activity. The causal chain runs purchasing power → consumption → labour reallocation → sector entry → capital returns. Sub-mechanisms: (1) increased household consumption, especially of services with high domestic labour content; (2) labour reallocation as the minimum-income threshold required for a decent standard of living falls, expanding the set of acceptable jobs; (3) entry into previously marginal sectors, viable simultaneously from the demand side (more customers) and the supply side (workers can accept lower wages); (4) returns shifting across capital types, with productive capital in expanding sectors gaining and rent-dependent or scarcity-dependent capital losing; (5) distributional ambiguity — the mechanism produces structural reallocation, not uniform improvement. The hypothesis is explicitly flagged as a research question for Phase One, not a settled position. (LDW §4.5)

**What the paper cannot establish.** The purchasing power figures assume full displacement actualises. Whether full displacement occurs depends on Governing Council calibration decisions, Phase One behavioural response data, and the LRR timeline. The infrastructure investment case in §4.3 is the author's normative argument, not settled design. The demand expansion hypothesis in §4.5 is speculative by intent and requires Phase One measurement. The welfare demand effects in §6 are directional without magnitudes. (LDW §7)
...
Establishes death, jurisdictional exit, threshold fall-through, and bankruptcy as a unified class of event, and derives the bridging facility and re-entry rule from that unification. The paper's conceptual contribution is the abstraction itself: shifting from a conventional exit-tax framing to a general account of how a delta-based assessment position closes.

**Prior literature rejected.** Conventional exit tax design treats departure as an avoidance act requiring fiscal deterrence. The wealth-tax migration literature treats emigration as a behavioural response to be mitigated. The insolvency literature treats tax obligations as one creditor claim among many. None asks the prior question: what does a mechanism built on an ongoing assessment relationship owe the individual when that relationship ends, and what does the individual owe the mechanism? CLOSE proposes that answering it produces a general framework within which all four closure types are instances of the same underlying event. (CLOSE §1.4)
...
**What the mechanism owes.** Three obligations apply regardless of closure event type: (1) a correct final delta calculation with no haircut or penalty by reason of the closure type; (2) the symmetric refund on any negative final delta — the state's downside commitment does not diminish because the individual is dying, departing, or insolvent; (3) no punitive exit taxation. Exit closure is not an act requiring coercive fiscal response; the enforcement work is done by the annual reporting trail and correct settlement, not a border penalty. A punitive exit provision would signal that the mechanism does not trust its own design to retain participants. The full response to the Agrawal cross-base externality argument for punitive exit — including the Phase Two disappearance of that externality once parallel taxes are displaced — is in CLOSE §3 and BEHAV.A §D. (CLOSE §3)

**The four closure event types.**

- *Death.* No structural addition required beyond the inheritance auction machinery in VAL §6.5: Route D assets auctioned at the declared value as the opening bid floor; the heir enters at the auction price as their opening basis. Routes A/B close through professional valuation at death; Route C through the must-transfer mechanism. (CLOSE §4.1)

- *Jurisdictional exit.* The bridging facility (CLOSE §5) decouples physical departure from settlement completion. Route D is the substantively novel case: the exit valuation service opens on the declared exit date using the same auction infrastructure as the inheritance and mid-position auctions — open bidding at the declared value as floor, only third parties may bid, taxpayer holds right of first refusal, winning bid becomes the new recognised basis. A taxpayer who has reported honestly has a well-established floor the auction is likely to confirm; a systematic understater finds the floor is low because they set it. (CLOSE §4.2)

- *Threshold fall-through.* The final delta is the distance from the last declared basis to the exemption threshold W_min — a negative delta in all cases; the symmetric refund fires on that amount. The threshold is both the trigger and the terminal value: the WDT's interest runs only to the threshold in either direction. This eliminates the position-suspension problem and all re-entry mechanics complexity. (CLOSE §4.3)

- *Bankruptcy.* Position closes under insolvency practitioner direction. The WDT follows jurisdiction-specific creditor priority (creditors first, WDT claim subordinate): the WDT's purpose is taxing gains, not competing for residue of a failed estate. The refund obligation is not subject to creditor priority: if the final delta is negative (likely in a bankruptcy closure), the SWF owes the estate a guaranteed refund that flows into the creditor waterfall. A current Custodian mandate gap — GOV.B §E does not specify who initiates the refund calculation when the taxpayer is under insolvency administration — must be resolved before Phase One, as the first bankruptcy closure will otherwise face a procedural gap at exactly the moment the symmetric commitment most needs to be seen to operate. (CLOSE §4.4)

**Bridging facility.** Decouples physical departure from settlement completion. Where the expected exit delta is positive the taxpayer posts a bond; where negative the SWF posts a bond to the taxpayer as security for the expected refund; where uncertain both sides post proportional bonds that net on settlement. Friction concentrates on non-cooperation, not on the act of departure. Bond sizing ratios are Governing Council calibration parameters under the Tier 1 process, informed by Phase One exit data; the principles are in GOV.B §E.3. (CLOSE §5)

**Re-entry rule.** The re-entry basis is the individual's net worth at the point of re-entry (fresh start on the basis). The lifetime envelope carries the prior history forward. Two components work differently and must be stated separately: the basis resets; the envelope does not. This closes the strategic cycling loophole — a re-entrant who received large prior refunds enters with reduced refund headroom, proportional to what they already received — while also making re-entry attractive to long-term net contributors, who carry a larger envelope balance and therefore more refund headroom than a first-time entrant of equivalent wealth. The re-entry rule's Phase Two extension to foreign nationals with prior WDT-equivalent positions in other jurisdictions requires bilateral information exchange arrangements and cannot be designed unilaterally. (CLOSE §6)

**Beyond-lifetime-cap exploitation surface.** If refunds could exceed cumulative lifetime contributions, a taxpayer could accumulate a modest tax history, engineer a large paper loss, receive a net refund, and exit with the position legally settled. The lifetime cap makes this impossible by construction: the mechanism can only return what it has collected. The cap is not a restriction on the symmetric refund commitment — it is a removal of the incentive to manufacture losses. Any Governing Council extension of refund availability beyond the cap is a named trade-off under the MF §9 framework and must specify the enforcement mechanism required to close the exploitation surface before the extension is offered. (CLOSE §8.4)
...
Examines whether the WDT creates a structural competitive dynamic between governance system types — specifically, whether democratic systems implementing a full WDT would gain access to a pool of private wealth that authoritarian systems are structurally excluded from. The paper's central contribution to the taxation→representation literature is the **two-constituency mechanism**: two non-overlapping constituencies converging on identical governance requirements, structurally resistant to executive division.

**The two-constituency mechanism.** The WDT creates two non-overlapping constituencies whose demands converge on identical governance requirements. Wealthy taxpayers require credible protection of the symmetric refund and valuation architecture — commitments requiring constitutional constraint on executive discretion. The working majority, once the labour dividend is flowing, acquires a direct financial interest in the mechanism's continuation — creating demand for transparency and independent oversight. The convergence generates a cross-class accountability coalition qualitatively different from single-constituency representation pressure in the prior literature. Any executive attempt to separate the constituencies requires simultaneously offering wealthy taxpayers weaker refund protection (which collapses cooperative declaration and the revenue base the dividend depends on) or reducing the majority's benefit (which removes the fiscal source of the dividend). The first destroys the revenue base; the second removes the dividend itself. Neither offer can be made credibly without breaking the mechanism generating the revenue. DR is the constitutional instantiation of this mechanism: a lottery-drawn chamber from the general population, holding 50% of Governing Council vote share by derivation from the anti-collusion guarantee, is the working majority seated as a veto player within the fiscal mechanism itself — not a political force expected to arise, but an architectural requirement. (INST §6.2; GOV §3.2)

**The untaxed pool.** Approximately $286 trillion in global household wealth sits in Tier 4 and Tier 5 governance systems (primarily electoral autocracy and closed autocracy) that cannot access it cooperatively through the self-defeating dynamic: the wealth-holding population in those systems is also the governing coalition's support base, making large-scale cooperative wealth taxation equivalent to regime suicide. A further $228 trillion in Tier 1 and Tier 2 systems has been inaccessible through prior wealth tax mechanisms for the three political failure reasons POL identifies. The $286 trillion figure comprises three independently contestable components: the global wealth total (~$570 trillion, from McKinsey and UBS), the governance distribution (~50% in Tier 4/5, from country-level data mapped to V-Dem classifications), and the accessibility classification (Tier A and Tier B respectively). A critic must identify which component fails and by how much. (INST §3.2)

**Four conditions for a stable large-scale wealth tax.** (1) No accurate external valuation required — the WDT's self-declaration architecture; (2) cooperation must be the path of least resistance — the tolerant zone and must-transfer rule; (3) the refund guarantee must be constitutionally protected against regime discretion — the **fiscal sovereignty test**: for an authoritarian state to credibly implement the WDT, a non-executive institution must possess effective blocking authority over the tax rate, the refund obligation, reserve deployment, and amendment of the governing rules; if it does, the executive cannot unilaterally determine how its primary revenue instrument operates, and an institution approaching parity with total managed expenditure cannot be blocked without redistributing fiscal sovereignty; (4) the taxable population must not be the governing coalition — Condition 4 is enforced architecturally by DR's 50% vote share rather than by political dynamics that may or may not materialise. (INST §5)

**Three-stage exclusion argument.** Predatory and electoral autocracies face hard structural exclusion through the self-defeating dynamic. Developmental authoritarian systems (the genuinely difficult case) face endogenous institutional pressure: attempting to satisfy all four conditions generates the two-constituency accountability coalition and narrows the institutional distance between "stable WDT equilibrium" and democratic governance with each assessment cycle. Full WDT implementation requires constitutionally seating DR as a design requirement, making the endogenous pressure partly structural rather than only dynamic. China's trajectory from the Deng-to-Hu period to the post-2012 Xi era illustrates this as a case consistent with the predicted mechanisms — not a confirmed pattern from a single case: the August 2026 National Defence Mobilisation Law revision explicitly reserves executive authority over economic resources in circumstances the state defines, creating a governing hierarchy of claims under which any notional refund guarantee operates, making Condition 3 and the refund guarantee structurally incompatible in statute. The WDT without the labour dividend is not a stable authoritarian equilibrium either: it fails to create the majority constituency that provides organised defence against the three failure mechanisms POL identifies, and removes the recirculation through which wealthy taxpayers' WDT contributions return through improved operating environments and consumer demand. (INST §6)

**Three competitive advantages for democratic WDT adopters.** Each stated as an explicit conditional with its falsification threshold: (1) the resource pool advantage obtains if and only if cooperative declaration produces revenue at the RATES lower bound and cross-base migration remains below the Agrawal six-to-one erosion threshold; (2) the welfare floor advantage — narrowing the authoritarian endurance asymmetry — obtains if and only if the ENV displacement channel produces disposable income improvement legible at the individual payslip level within one political cycle of LRR fill; (3) pre-built crisis capacity obtains if and only if the SRR and LRR accumulate to floor values before a major stress event requires drawdown. All three are Tier B claims requiring Phase One verification. (INST §7)
...
The complete record of what this project does not settle is maintained in SCOPE (v0.4, 24 September 2026), which is the canonical register for all open items. Items not listed in SCOPE are settled.

**SCOPE §2 — Comparative political economy and international coordination (#7–#9, #31–#32).** Three confirmed literature gaps requiring formal comparative political science methods: causal framework validation for the three-mechanism account of wealth tax abolition (#7); international competitive dynamic at the political level (#8); minimum-tax floor interaction with the symmetric loss-refund (#9). Two items added in SCOPE v0.4 from INST: endogenous institutional pressure on developmental authoritarian states attempting to satisfy all four WDT conditions (#31); adoption diffusion dynamics under competitive pressure (#32). Items #31 and #32 are Tier C claims in INST's taxonomy; they require both Phase One outcomes and subsequent comparative observation.

**SCOPE §3 — Phase One empirical questions (#4–#6, #13–#16, #18, #29–#30).** Ten items unanswerable before a live system: cooperative compliance at ultra-high-net-worth level (#4); cross-base migration externality magnitude (#5); administrative-layer intervention effects (#6); valuation route adoption distribution and flexibility levy calibration (#13); OBR independence as mandate-guardian (#14); corporate instrument transition conditions (#15); housing price net effect (#16); SRR floor calibration under mild-overstatement equilibrium (#18); monitoring instrument for the population distribution of α (#29); bootstrapping problem Phase One vulnerability window (#30). FAL has given several of these precise falsification structure but they remain empirically open. Two pre-Phase-One analytical deliverables added in SCOPE v0.4 are closable with existing data and model infrastructure: Route D realisation event flow and LRR capitalisation window impact (#33, assigned to RATES.A extension); population-weighted revenue calculation across the predicted α distribution (#34, assigned to RATES.A extension and SWEEPS follow-on).

**SCOPE §4 — MACRO successor tasks (#11–#12).** Consumption multiplier magnitude and net bias direction of RATES estimates (#11); automation and tax-base migration under different automation trajectories (#12). Both require Phase One calibration data; both are addressed in ENV and FAL H2/H5 as open empirical inputs.

**SCOPE §5 — Governing Council calibration parameters open in value (#10, #17, #19–#21, #24).** Settled in kind; open in value pending Phase One data. The τ_0 × W_min joint surface (#17) is the only item in this group resolvable without Phase One data and is the next analytical deliverable.

**SCOPE §6 — Jurisdiction-specific and institutional preconditions (#22–#23, #25–#28).** Not design gaps; resolution depends on legal analysis, diplomatic process, or institutional negotiation specific to a jurisdiction. The constitutional and legal analysis of the Route D auction trigger, exit closure, and bridging facility interaction with deemed-disposal rules (#28) is a precondition for implementation in any specific jurisdiction.