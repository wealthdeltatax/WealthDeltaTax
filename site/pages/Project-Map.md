...
**Five friction types** (information uncertainty, visibility of reciprocity, legitimacy perception, feedback responsiveness, compliance cost) produce five design principles. The Membrane — the layer through which taxpayers experience the institution — has five health dimensions with a specific interaction structure: Clarity and Accessibility are most complementary; Fairness is downstream of all four other dimensions and is the leading indicator of membrane failure.

**Nine behavioural shapes** from full cooperative compliance to active resistance provide the range against which robustness is evaluated. Shapes 4–6 (restructuring, timing manipulation, cross-border asset migration without personal exit) generate most value at risk from avoidance. The enforcement paradigm shift is structural: the state does not need to discover true values — unattributed ownership faces τ_h by default, and surfacing hidden assets compounds the concealment cost progressively.

**Cross-base externality.** The Agrawal et al. (2025) six-to-one multiplier (income tax and VAT losses approximately six times the direct wealth-tax revenue loss) applies during Phase One when existing taxes run alongside the WDT. Six design responses: the multiplier is a point estimate from subnational Spanish data, not a structural constant; membrane investment is fiscally urgent precisely because of the multiplier; the Governing Council can reduce rates in response to observed departure; the LRR timeline extension buffer allows lower-rate operation; the re-entry rule and envelope carry-forward strengthen with system age; net inward migration remains a formal possibility.
