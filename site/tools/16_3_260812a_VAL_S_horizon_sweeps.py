...
def fig_n_crossing_annotated():
    ...
    for alpha, col, style in zip(OVER_ALPHAS, cols, ls):
        diffs = []
        for n in N_sweep:
    ...
    ax1.axvline(CANON_N, color='#888888', linewidth=1.0, linestyle=':',
                label=f'N = {CANON_N} (RATES ref)')
    ax1.text(CANON_N + 0.5, ax1.get_ylim()[0] * 0.85,
             f'N = {CANON_N}', fontsize=8, color='#666666')
    ax1.fill_between(N_sweep, [0] * len(N_sweep), [min(0, d) for d in
                     [sum([run_sim(p, alpha=1.5, g=CANON_G, N=n)['Net'] -
                           run_sim(p, alpha=1.0, g=CANON_G, N=n)['Net'] for alpha in [1.5]])
    ...
    x = range(len(OVER_ALPHAS))
    bars = ax2.bar(x, [cn if cn is not None else 0 for cn in crossing_ns],
                   color=cols, edgecolor='white', width=0.5)
    ax2.axhline(CANON_N, color='#888888', linewidth=1.5, linestyle='--',
                label=f'N = {CANON_N} (RATES ref)')

    for bar, alpha, nc in zip(bars, OVER_ALPHAS, crossing_ns):
        if nc is not None:
...
def fig_n_understater_panels():
    ...
    set_style()
    N_VALS  = N_PANEL_VALS
    N_LABEL = [f'N = {n}' + (' (canonical)' if n == CANON_N else '') for n in N_VALS]
    ...
    for ax, n, label in zip(axes, N_VALS, N_LABEL):
        p = make_p(N=n)
        for alpha, col in zip(UNDER_ALPHAS, under_cols):
        ...
        ax.axvline(CANON_G * 100, color='#888888', linewidth=0.8, linestyle='--')
        ax.text(CANON_G * 100 + 0.2, 2, f'g = {CANON_G*100:.1f}%', fontsize=7.5, color='#666666')
        ax.set_xlabel('g (%)')
        ax.set_ylabel('C.1 (pp)')
        ax.set_title(label, fontsize=10, fontweight='bold' if n == CANON_N else 'normal')
        ax.set_xlim(0, 35)
        ax.legend(fontsize=7.5)

    fig.suptitle(
        'Fig S3.1b — Understater penalty profile across holding periods\n'
        f'τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, k={CANON_K}, V₀=£{CANON_V0:.0f}m  ·  Bold = canonical',
        fontsize=10
    )
    plt.tight_layout()
...
def fig_n_tolerant_zone():
    ...
    ax.axvline(CANON_N, color='#aaaaaa', linewidth=1.0, linestyle=':',
               label=f'N = {CANON_N} (RATES ref)')

    ax.set_xlabel('Holding period N (years)')
    ax.set_ylabel('Declaration ratio α')
    ax.set_title(
        f'Fig S3.1c — Tolerant zone (|C.1| < {TZONE_THRESHOLD*100:.0f}pp) stability across N\n'
        f'τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, k={CANON_K}, g={CANON_G*100:.1f}%'
    )
    ax.legend(fontsize=7.5)
    ax.set_xlim(5, 65)
    ax.set_ylim(0.5, 2.2)
...
V0_VALS   = V0_VALS   # imported from val_s_helpers
V0_LABELS = [f'V₀ = £{v:.0f}m' + (' (canonical)' if v == CANON_V0 else '')
             for v in V0_VALS]
...
def fig_v0_c1_curves():
    ...
    for v0, label, col in zip(V0_VALS, V0_LABELS, V0_COLS):
        p = make_p(V0_m=v0)
        c1_vals = [c1(p, a, CANON_G) * 100 for a in alpha_fine]
        lw = 2.2 if v0 == CANON_V0 else 1.6
    ...
    ax.set_title(
        ...
        f'τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, k={CANON_K}, N={CANON_N}, g={CANON_G*100:.1f}%'
    )
...
def fig_v0_entry_rate():
    ...
    p = make_p()
    W_range = np.logspace(np.log10(CANON_WMIN), np.log10(2000), 500)
    ...
    ax.text(1500, CANON_TAU0 * 100 + 0.5, f'τ₀ = {CANON_TAU0*100:.0f}%', fontsize=8, color='#888888', ha='right')
    ax.text(1500, CANON_TAUM * 100 - 0.5, f'τ_m = {CANON_TAUM*100:.0f}%', fontsize=8, color='#888888', ha='right', va='top')
    ...
    ax.set_title(
        ...
        f'τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, k={CANON_K}, W_min=£{CANON_WMIN:.0f}m'
    )
    ax.set_xlim(CANON_WMIN, 2000)
    ax.set_ylim(0, CANON_TAUM * 100 * 1.15)
...
def fig_v0_heatmaps():
    ...
    for ax, v0, label in zip(axes, V0_VALS, V0_LABELS):
        p = make_p(V0_m=v0)
        ...
        ax.set_title(label, fontsize=10, fontweight='bold' if v0 == CANON_V0 else 'normal')
    ...
    fig.suptitle(
        ...
        f'τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, k={CANON_K}, N={CANON_N}  ·  Bold = canonical',
        fontsize=10, y=1.01
    )
...
def main():
    ...
    print(f'\nScript 2 complete. Figures in {OUT_DIR}')
