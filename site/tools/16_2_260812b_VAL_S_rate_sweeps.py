...
TAU0_LABELS = [f'τ₀ = {v*100:.0f}%' + (' (canonical)' if v == CANON_TAU0 else '')
               for v in TAU0_VALS]
...
def fig_tau0_heatmaps():
    ...
    for ax, tau_0, label in zip(axes, TAU0_VALS, TAU0_LABELS):
        ...
        ax.set_title(label, fontsize=10, fontweight='bold' if tau_0 == CANON_TAU0 else 'normal')
        # Cell annotations
        ...
        hon = ALPHA_VALS.index(1.0)
        ax.add_patch(plt.Rectangle((-0.5, hon - 0.5), len(G_VALS), 1,
...
def fig_tau0_n_crossings():
    ...
    for alpha in OVER_ALPHAS:
        ...
        ax.plot(
            [t * 100 for t in TAU0_VALS], crossings,
            color=col_by_alpha[alpha], linestyle=ls_by_alpha[alpha],
            linewidth=2, marker='o', markersize=5,
            label=f'α = {alpha}'
        )

    ax.axhline(CANON_N, color='#888888', linewidth=0.9, linestyle=':', label=f'N = {CANON_N} (RATES ref)')
    ax.axvline(CANON_TAU0 * 100, color='#888888', linewidth=0.9, linestyle=':')
    ax.text(CANON_TAU0 * 100 + 0.3, ax.get_ylim()[0] + 1, f'τ₀ = {CANON_TAU0*100:.0f}% (canonical)',
...
def fig_tau0_tolerant_zone():
    ...
    ax.axvline(CANON_TAU0 * 100, color='#888888', linewidth=0.9, linestyle=':', label=f'Canonical τ₀ = {CANON_TAU0*100:.0f}%')

    ax.set_xlabel('Entry rate τ₀ (%)')
    ax.set_ylabel('Declaration ratio α')
    ax.set_title(
        f'Fig S2.1c — Tolerant zone (|C.1| < {TZONE_THRESHOLD*100:.0f}pp) boundaries by τ₀\n'
        f'τ_m = {CANON_TAUM*100:.0f}%, k = {CANON_K}, N = {CANON_N}, g = {CANON_G*100:.1f}%'
    )
    ax.legend()
    ax.set_xlim(5, 44)
    plt.tight_layout()
...
TAUM_LABELS = [f'τ_m = {v*100:.0f}%' + (' (canonical)' if v == CANON_TAUM else '')
               for v in TAUM_VALS]
...
def fig_taum_heatmaps():
    ...
    for ax, tau_m, label in zip(axes, TAUM_VALS, TAUM_LABELS):
        ...
        ax.set_xlabel('g', fontsize=8)
        ax.set_ylabel('α', fontsize=8)
        ax.set_title(label, fontsize=10, fontweight='bold' if tau_m == CANON_TAUM else 'normal')
        for i in range(len(ALPHA_VALS)):
            for j in range(len(G_VALS)):
                v = mat[i, j]
                col = 'white' if abs(v) > vmax * 0.6 else '#1a1a1a'
    ...
    fig.suptitle(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, k = {CANON_K}, N = {CANON_N}, V₀ = £{CANON_V0:.0f}m  ·  Bold = canonical',
        fontsize=10, y=1.01
    )
    plt.tight_layout(rect=[0, 0, 0.91, 1])
...
def fig_taum_penalty_plateaus():
    ...
    for tau_m, ls in ls_by_taum.items():
        ...
        ax.plot(
            ...
            label=f'τ_m = {tau_m*100:.0f}%' + (' (canonical)' if tau_m == CANON_TAUM else '')
        )
    ...
    ax.set_title(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, k = {CANON_K}, N = {CANON_N}, g sweep 18–40%'
    )
    ax.legend()
    ax.set_xlim(5, 85)
    plt.tight_layout()
...
def fig_taum_n_crossings():
    ...
    ax.axhline(CANON_N, color='#888888', linewidth=0.9, linestyle=':', label=f'N = {CANON_N} (RATES ref)')
    ax.axvline(CANON_TAUM * 100, color='#888888', linewidth=0.9, linestyle=':')
    ax.text(CANON_TAUM * 100 + 0.5, ax.get_ylim()[0] + 1,
            f'τ_m = {CANON_TAUM*100:.0f}% (canonical)', fontsize=7.5, color='#666666')
    ...
    ax.set_title(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, k = {CANON_K}, V₀ = £{CANON_V0:.0f}m, g = {CANON_G*100:.1f}%'
    )
    ax.legend()
    plt.tight_layout()
...
K_LABELS = [f'k = {v}' + (' (canonical)' if v == CANON_K else '') for v in K_VALS]
...
def fig_k_rate_curves():
    ...
    W_vals = np.logspace(np.log10(CANON_WMIN), np.log10(5000), 400)

    for k_val, label, col in zip(K_VALS, K_LABELS, K_COLS):
        p = make_p(k=k_val)
        tau_vals = [tau(w, p) * 100 for w in W_vals]
        lw = 2.2 if k_val == CANON_K else 1.6
        ax.semilogx(W_vals, tau_vals, color=col, linewidth=lw,
                    label=label, linestyle='-' if k_val == CANON_K else '--')

    ax.axvline(CANON_V0, color='#888888', linewidth=1.0, linestyle=':', label=f'V₀ = £{CANON_V0:.0f}m (reference)')
    ax.axhline(CANON_TAU0 * 100, color='#aaaaaa', linewidth=0.7, linestyle='--')
    ax.axhline(CANON_TAUM * 100, color='#aaaaaa', linewidth=0.7, linestyle='--')
    ax.text(4000, CANON_TAU0 * 100 + 0.8, f'τ₀ = {CANON_TAU0*100:.0f}%', fontsize=8, color='#888888', ha='right')
    ax.text(4000, CANON_TAUM * 100 - 0.8, f'τ_m = {CANON_TAUM*100:.0f}%', fontsize=8, color='#888888', ha='right', va='top')
    ...
    ax.set_title(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, τ_m = {CANON_TAUM*100:.0f}%, W_min = £{CANON_WMIN:.0f}m'
    )
    ax.set_xlim(CANON_WMIN, 5000)
    ax.set_ylim(0, CANON_TAUM * 100 * 1.12)
...
def fig_k_heatmaps():
    ...
    for ax, k_val, label in zip(axes, K_VALS, K_LABELS):
        p = make_p(k=k_val)
        mat = c1_matrix(p)
        ax.grid(False)
        ...
        ax.set_xlabel('g', fontsize=8)
        ax.set_ylabel('α', fontsize=8)
        ax.set_title(label, fontsize=10, fontweight='bold' if k_val == CANON_K else 'normal')
        for i in range(len(ALPHA_VALS)):
            for j in range(len(G_VALS)):
                v = mat[i, j]
                col = 'white' if abs(v) > vmax * 0.6 else '#1a1a1a'
    ...
    fig.suptitle(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, τ_m = {CANON_TAUM*100:.0f}%, N = {CANON_N}, V₀ = £{CANON_V0:.0f}m  ·  Bold = canonical',
        fontsize=10, y=1.01
    )
    plt.tight_layout(rect=[0, 0, 0.91, 1])
...
def fig_k_bracket_penalty():
    ...
    for V0, col in zip(V0_points, cols):
        ...
        for k_val in k_fine:
            ...
            c1_vals.append(c1(p, alpha=1.8, g=CANON_G) * 100)
    ...
    ax.axvline(CANON_K, color='#888888', linewidth=0.9, linestyle=':',
               label=f'Canonical k = {CANON_K}')
...
WMIN_LABELS = [f'W_min = £{v:.0f}m' + (' (canonical)' if v == CANON_WMIN else '')
               for v in WMIN_VALS]
...
def fig_wmin_rate_curves():
    ...
    for w_min, label, col in zip(WMIN_VALS, WMIN_LABELS, WMIN_COLS):
        p = make_p(W_min=w_min)
        tau_vals = [tau(w, p) * 100 for w in W_vals]
        lw = 2.2 if w_min == CANON_WMIN else 1.6
        ax.semilogx(W_vals, tau_vals, color=col, linewidth=lw,
                    label=label, linestyle='-' if w_min == CANON_WMIN else '--')

    ax.axvline(CANON_V0, color='#888888', linewidth=1.0, linestyle=':',
               label=f'V₀ = £{CANON_V0:.0f}m (reference)')
    ax.axhline(CANON_TAU0 * 100, color='#aaaaaa', linewidth=0.7, linestyle='--')
    ax.axhline(CANON_TAUM * 100, color='#aaaaaa', linewidth=0.7, linestyle='--')
    ax.text(4000, CANON_TAU0 * 100 + 0.8, f'τ₀ = {CANON_TAU0*100:.0f}%',
            fontsize=8, color='#888888', ha='right')
    ax.text(4000, CANON_TAUM * 100 - 0.8, f'τ_m = {CANON_TAUM*100:.0f}%',
            fontsize=8, color='#888888', ha='right', va='top')

    # Annotate each W_min onset with a short vertical tick
    for w_min, col in zip(WMIN_VALS, WMIN_COLS):
...
def fig_wmin_heatmaps():
    ...
    for ax, w_min, label in zip(axes, WMIN_VALS, WMIN_LABELS):
        p = make_p(W_min=w_min)
        mat = c1_matrix(p)
        ax.grid(False)
        ax.imshow(mat, aspect='auto', cmap='RdBu_r', norm=norm, zorder=2)
        ...
        ax.set_title(label, fontsize=10,
                     fontweight='bold' if w_min == CANON_WMIN else 'normal')
        for i in range(len(ALPHA_VALS)):
            for j in range(len(G_VALS)):
                v = mat[i, j]
                col = 'white' if abs(v) > vmax * 0.6 else '#1a1a1a'
    ...
    fig.suptitle(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, τ_m = {CANON_TAUM*100:.0f}%, k = {CANON_K}, '
        f'N = {CANON_N}, V₀ = £{CANON_V0:.0f}m  ·  Bold = canonical',
        fontsize=10, y=1.01
    )
    plt.tight_layout(rect=[0, 0, 0.91, 1])
...
def fig_wmin_n_crossings():
    ...
    ax.axhline(CANON_N, color='#888888', linewidth=0.9, linestyle=':',
               label=f'N = {CANON_N} (RATES ref)')
    ax.axvline(CANON_WMIN, color='#888888', linewidth=0.9, linestyle=':')
    ax.text(CANON_WMIN + 0.15, ax.get_ylim()[0] + 1,
            f'W_min = £{CANON_WMIN:.0f}m (canonical)', fontsize=7.5, color='#666666')
    ax.axvline(CANON_V0, color='#cccccc', linewidth=0.8, linestyle='--',
               label=f'V₀ = £{CANON_V0:.0f}m (reference wealth)')
    ...
    ax.set_title(
        ...
        f'τ₀ = {CANON_TAU0*100:.0f}%, τ_m = {CANON_TAUM*100:.0f}%, k = {CANON_K}, '
        f'V₀ = £{CANON_V0:.0f}m, g = {CANON_G*100:.1f}%'
    )
    ax.legend()
    ax.set_xlim(0.5, 11)
    plt.tight_layout()
...
def main():
    ...
    print(f'Canonical (from TOML): τ₀={CANON_TAU0*100:.0f}%, τ_m={CANON_TAUM*100:.0f}%, '
          f'k={CANON_K}, W_min=£{CANON_WMIN:.0f}m, N={CANON_N}, '
          f'V₀=£{CANON_V0:.0f}m, g={CANON_G*100:.2f}%')
    ...
    print(f'\nScript 1 complete. Figures in {OUT_DIR}')
