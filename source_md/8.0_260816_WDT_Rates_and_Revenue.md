...
**Logistic rate function:** The functional form $\tau$(W) applied to annual wealth deltas. Rises smoothly from $\tau_0$ at W_min to an asymptotic ceiling of $\tau_m$. Specified by four parameters: $\tau_0$, $\tau_m$, k, and W_min. See (RATES §4) for the full expression.
...
The 2007 reference scenario establishes what the mechanism delivers under the hardest illustrative case in the UK equity return dataset. To test whether the revenue and solvency results depend on that choice, the SSM was run at Balanced parameters from every viable start year in the 1947–2019 UK equity return data. The full 73-row sweep table and statistical pass are in (RATES.A §B.4) and (RATES §B.5); key findings are presented here.
