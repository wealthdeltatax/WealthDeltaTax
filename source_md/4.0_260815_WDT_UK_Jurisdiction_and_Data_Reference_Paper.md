...
**Rate function parameters (`tau_0`, `tau_m`, `k`, `W_min`).** Design parameters, not empirically derived quantities. The canonical values (τ₀ = 15%, τ_m = 70%, k = 0.001 per £m, W_min = £2m) constitute the Balanced scenario used throughout RATES. Justification and full sensitivity analysis across the parameter space are in SWEEPS and SWEEPS.A.
