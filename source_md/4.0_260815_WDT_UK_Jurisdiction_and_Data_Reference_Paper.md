...
| 1.3      | 07 July 2026   | Cross-reference corrections: GOV §7.5 → GOV §6.3 (mandate guardian, three occurrences in §1.5.3 and §4.4); GOV §7.3 → GOV.B §E.6 (institutional mapping, §1.5.3) |
...
A reference jurisdiction grounds the companion papers without constraining the whitepaper. Without one, RATES revenue modelling and the behavioural response work in WP §11.2 remain notional; with one, they become concrete tasks with defined data sources.
...
Where Advani et al. (2020) data and analysis remain useful, this paper draws on them. Their revenue modelling, threshold sensitivity analysis, and administrative cost evidence are the most directly relevant prior work for the microsimulation agenda in WP §11.1.
...
GOV §6.3 identifies the SWF Custodian as the WDT's mandate guardian and specifies what that role requires: statutory independence, long overlapping non-renewable tenure, stewardship statements creating a ratcheting reputational commitment, outputs in the permanent public record, and no role in setting the inputs against which its performance is scored. The OBR model (a small expert body with TSC oversight of appointments) is the most directly relevant UK precedent. In the UK reference jurisdiction, the Custodian maps to the Bank of England or a dedicated statutory body on the Bank's independence architecture (GOV.B §E.6). Whether the actual OBR meets the structural standard GOV §6.3 specifies, tested against the 2022 mini-budget period and the December 2025 resignation, is assigned to Phase One at §4.4.
...
The primary source is the ONS WAS. As noted in §1.2.1, WAS lost Official Statistics accreditation in June 2025 and should be treated with caution. Advani et al. (2020) IFS work provides partial corrections at the top of the distribution.
...
The following gaps should be addressed before the revenue microsimulation in WP §11.1 is finalised.
...
Note: JUR's body text retains cross-references to old GOV section numbers (§8.6 and §8.4) predating the GOV v2.1 restructuring. These are stale but benign; the content exists at its new locations.
