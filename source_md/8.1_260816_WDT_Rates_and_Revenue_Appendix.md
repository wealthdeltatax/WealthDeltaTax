...
*SSM coverage ratio: average annual SSM net income over the capitalisation window (SRR fill to LRR breakeven) divided by average annual expenditure over the same window. The SSM applies uniform historical returns across the population (correlated-shock assumption); TCM coverage appears in (RATES.A §3).*
