...
Wealth Delta Tax · position closure · closure event · death · exit · threshold fall-through · bankruptcy · bridging facility · re-entry · lifetime contribution envelope · cooperative architecture

---

### Author Disclosure {.unnumbered .unlisted}

The author used AI assistance (Claude, Anthropic) in drafting this paper.
...
| 0.3 | 24 July 2026 | Intellectual context section added (§1). No-punitive-exit-taxation position added to (CLOSE §3.) Re-entry incentive paragraph added to (CLOSE §6.) Cross-base externality Phase One/Two distinction clarified in (CLOSE §9.) Section numbering updated throughout. |
| 0.4 | 25 July 2026 | (CLOSE §2) general theory updated to note three-context auction infrastructure. (CLOSE §4.2) updated with cross-reference to (CLOSE §1.0) compelled mid-position auction as third instance of shared Route D auction architecture. |
| 0.5 | 30 July 2026 | Formatter pass: Internal Bibliography updated (VAL.A retitled; VAL.B added). (CLOSE §2) past-solution tell removed. Em dashes converted to parentheses throughout. (CLOSE §3) duplicate honest-declarant passage removed; no-punitive-exit paragraph tightened. (CLOSE §4.3) two-properties opener condensed. (CLOSE §5) bridging facility description updated to reflect GOV v1.4 (CLOSE §8.8) symmetric bond structure; forward-reference to (CLOSE §8.8) added. (CLOSE §6) verbosity reduced; retrospective inevitability removed from re-entry incentive paragraph. (CLOSE §9) BEHAV re-summary condensed. CLOSE OQ 1 closure notice added. |
...
| 0.8 | 14 Aug 2026 | Sanity-check corrections: header version corrected (was stuck at v0.5); (CLOSE §2) and (CLOSE §4.2) (CLOSE §1.0) cross-references updated to (CLOSE §6.1) / (GOV.B §G); (CLOSE §2) and (CLOSE §4.2) auction description corrected — taxpayer excluded from bidding, holds right of first refusal at highest third-party bid (was incorrectly described as entitled to bid alongside third parties); (CLOSE §5) cross-reference updated from (CLOSE §8.8) to (GOV.B (CLOSE §E.3).) |
...
**Exit valuation service:** The taxpayer-initiated process that opens formal valuation on Route D assets at the declared exit date, using the inheritance-auction machinery described in (VAL (CLOSE (CLOSE §6).)5), with the taxpayer's own declared annual values serving as the opening bid floor.
...
The mechanism owes the individual (or their estate) the symmetric refund on any negative final delta. If the terminal net worth is below the prior declared basis, the refund obligation fires at the applicable marginal rate. The state's commitment to accept downside exposure alongside the taxpayer (M(F (CLOSE §7)); (GOV.A (CLOSE (CLOSE §E)).)2) does not diminish because the individual is dying, departing, or insolvent. The refund flows to whoever has the legal claim on the individual's assets at closure: their estate in the case of death, their creditor pool in the case of bankruptcy, the individual themselves in the other cases.
...
The cross-base fiscal externality identified by Agrawal, Foremny, and Martinez-Toledano (2025) (income tax and VAT losses approximately six times larger than the direct wealth-tax revenue loss from emigration) is sometimes cited as the fiscal justification for punitive exit taxation. Punitive exit provisions do not recover income tax and VAT base; they operate on the WDT's own base only. The externality is also a Phase One transitional exposure that resolves structurally in Phase Two when those parallel taxes are displaced by WDT revenue. The fuller treatment is in (CLOSE (CLOSE §9).) The response to the compulsion objection is in (CLOSE (CLOSE §8.1).)
...
Death is the closure event the WDT's existing machinery most fully addresses. (VAL (CLOSE (CLOSE §6).)5) specifies the inheritance auction mechanism in detail: Route D assets are auctioned to establish a final price, the taxpayer's declared annual values serve as the opening bid floor, and the heir enters the position at the auction price as their new basis. The delta for the final period is calculated against the prior declared basis; if the estate has experienced losses in the final period, the symmetric refund flows to the estate before distribution to heirs.
...
No structural addition is required for death beyond what (VAL (CLOSE (CLOSE §6).)5) already specifies. It is included in this paper's taxonomy because the general theory of position closure is the correct framing for understanding why the inheritance auction works the way it does: it is not a special inheritance mechanism but the standard position-closure settlement applied to the procedural circumstances of death.

## 4.2 Jurisdictional Exit

Jurisdictional exit (the taxpayer departing the jurisdiction and ceasing to be within the WDT's scope) is the closure event requiring the most structural addition: the bridging facility described in (CLOSE (CLOSE §5).) The settlement logic is identical to death (the position closes, the final delta is calculated, the mechanism's obligations are honoured). The procedural challenge is that the taxpayer is alive and mobile, so the timing of closure cannot be fixed by an external event the way death fixes it.

For Routes A and B, exit triggers professional valuation at the declared exit date, following the competitive-tender model in (VAL (CLOSE (CLOSE §10)).) For Route C, exit triggers the must-transfer settlement: the equity interest transfers at the declared value and the delta is calculated in the standard way. Route D is the substantively novel case, since Route D was designed precisely for assets where periodic cash settlement is impractical. The inheritance-auction mechanism applies directly: the exit valuation service opens on the declared exit date, the declared annual values serve as the opening bid floor, and the auction runs to establish the final price.

The same auction infrastructure (open bidding at the declared value as floor, only third parties may bid, the taxpayer holds a right of first refusal at the highest third-party bid, and the winning price becomes the new recognised basis) also operates in a third context: the (CLOSE (CLOSE §6.1)) / (GOV.B (CLOSE §G)) compelled mid-position revaluation, triggered when the three-body Valuation Body system confirms a Route D declaration as a significant outlier. That mechanism is not a closure event; the position continues after the auction. But it uses identical mechanics and the same Custodian administration, and its existence reinforces the exit auction's credibility: the infrastructure is a standing feature of the Route D architecture, not purpose-built for departure alone.
...
Londoño-Vélez, J., & Avila-Mahecha, J. (2024). *Behavioral responses to wealth taxation: Evidence from Colombia* (NBER Working Paper No. 32134). National Bureau of Economic Research. https://doi.org/10.3386/w32134
