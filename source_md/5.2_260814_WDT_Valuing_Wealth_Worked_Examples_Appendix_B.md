...
|:--------:|:----:|---------|
| 1.0 | 25 July 2026 | First draft. Split from VAL.A. Five examples constructed to illustrate the five claims in (VAL) (VAL.B §1), (VAL.B §5.2), (VAL.B §6.1), (VAL.B §6.4), and (VAL.B §7.1) respectively. Figures are placeholders pending Python model v1.0. |
| 1.1 | 26 July 2026 | Appendix lettering corrected: sections renumbered (VAL.B §J)–(VAL.B §O), continuing from (VAL.A (VAL.B §I).) Table captions updated. Reader's Guide table updated. Standalone (VAL.B §C.9) cross-references disambiguated to (VAL.A (VAL.B §C.)9) throughout. |
| 1.2 | 30 July 2026 | Python model v1.0 validation read-in: all [P] placeholder figures replaced with confirmed values across (VAL.B §J), (VAL.B §K), (VAL.B §L), (VAL.B §M), and (VAL.B §N.) Period-by-period tables added to (VAL.B §J) and (VAL.B §N.) (VAL.B §K) period-by-period accumulation table and key mechanism note added. (VAL.B §L) Timeline A and B structured tables added; inline £ figures confirmed. (VAL.B §M) model note added confirming g=5% compounded values ($V_10$ = £8.144m, $V_15$ = £10.395m); Option B/C prose figures confirmed. (VAL.B §N) three-founder period-by-period table and key findings section added. Reader's Guide placeholder notice removed. |
| 1.3 | 7 August 2026 | Parameter unification bug fix: all examples updated to canonical parameters k = 0.001, N = 34. (VAL.B §J) Table J.1 and period-by-period table replaced; key mechanism prose figures updated. (VAL.B §K) Table K.1 and period-by-period table replaced; key mechanism note updated. (VAL.B §L) Timeline A and B tables updated; comparison table updated. (VAL.B §M) Table M.1 updated. (VAL.B §N) Table N.1 and period-by-period table replaced; key findings figures updated. |
| 1.4 | 7 August 2026 | Plateau characterisation pass: (VAL.B §N.4) stale saturation reversal cross-reference removed and replaced with plateau characterisation; locally stable region framing corrected (honest declaration is zero-forecast-exposure baseline, not population equilibrium; equilibrium is mild overstatement per (VAL.A (VAL.B §A).)6). (VAL.B §N.5) extended: register inflation credit-market channel named as system-level consequence of mild-overstatement equilibrium; monitoring gap identified. |
| 1.5 | 13 August 2026 | Parameter update: $\tau_0$ = 15% (was 14%), k = 0.001 throughout (Reader's Guide previously cited k = 0.0001 in error). Reader's Guide parameters corrected. Table J.1, period-by-period (VAL.B §J), Table K.1 and period-by-period (VAL.B §K), (VAL.B §L) Timeline A/B tables and comparison table, (VAL.B §L) prose rate references, Table M.1 and (VAL.B §M) Option A/B/C prose, Table N.1 and period-by-period (VAL.B §N), and (VAL.B §N) Key findings all replaced with new model figures. |
...
Appendix sections are lettered (VAL.B (VAL.B §J)) through (VAL.B (VAL.B §O)), continuing from (VAL.A (VAL.B (VAL.B §F)).) Cross-references from (VAL) and (VAL.A) use the form "(VAL.B (VAL.B §J))", "(VAL.B (VAL.B §K))", and so on, to avoid ambiguity with VAL.A sections.

The five claims, and the examples that illustrate them, are:

| **Appendix** | **Claim in VAL** | **Asset** |
|:---|:---|:---|
| **(VAL.B §J): The deferred delta** | A declared value establishes the basis; understatement defers, it does not eliminate | Listed equity portfolio (Route C) |
| **(VAL.B §K): Dilution compounds with growth** | The must-transfer rule creates a cost that tracks the asset's trajectory | Founder stake, software company (Route C) |
| **(VAL.B §L): Why Route D defers to realisation** | Periodic cash settlement on illiquid assets would recreate forced-realisation pressure; in-kind settlement is impossible | Sculpture collection (Route D) |
| **(VAL.B §M): Voluntary settlement gives certainty, not avoidance** | Soft and hard basis resets are trade-offs, not escape hatches | Commercial property (Route D) |
| **(VAL.B §N): Honest declaration as the strategy with no forecast exposure** | Neither understatement nor overstatement is a dominant strategy; both require being right about the future to avoid a penalty | Private company — three founders, same asset (Route C) |

All figures are from Python model v1.0, validated against Excel 27 July 2026. (VAL.B (VAL.B §L)) and (VAL.B (VAL.B §M)) use closed-form arithmetic rather than run_val_sim; see individual section model notes for detail. All examples assume RATES-aligned parameters ($\tau_0$ = 15%, $\tau_m$ = 70%, W_min = £2m) and a marginal rate of approximately 15% at the relevant net worth level (k = 0.001, $V_0$ at or near £8–20m) unless stated otherwise. All values in £m unless stated otherwise.
...
**Claim illustrated:** A declared value establishes the recognised basis from which all future changes are measured. Understatement reduces current tax but enlarges the future delta; the deferral is recovered in full at realisation. (V(AL (VAL.B §1)), (VAL.B (VAL.B §5).)3)
...
The result holds at any positive growth rate. For the quantitative relationship between holding period, growth rate, and recovery size, see (VAL.A (VAL.B (VAL.B §C).)1) and (VAL.B (VAL.B §C.7).)

\newpage

# Dilution Compounds with Growth {.appendix}

**Claim illustrated:** For fungible assets under Route C, the must-transfer rule creates a cost that tracks the asset's trajectory. The faster the asset grows, the more expensive the understatement becomes; the dilution is an ongoing transfer at an underpriced rate, not a one-off payment. (V(AL (VAL.B (VAL.B §5)).)2)
...
The cost of understatement under Route C is not fixed at declaration. It grows with the asset. A founder who expects plateau may rationally consider a low declaration; a founder who expects strong growth takes on an increasingly expensive commitment with each window of understatement. This is the mechanism behind Proposition 3a in (VAL.A (VAL.B (VAL.B §A.5.3)).)
...
If the external benefits materially outweigh the additional WDT cost — if the company grew faster because of the credibility signal — then Founder C's strategy may have been rational even at a worse WDT outcome. The model in (VAL.A (VAL.B (VAL.B §C).)3) explores this through the β parameter.

At the system level, if mild overstatement is the rational equilibrium, declared values in the register are systematically above true values during holding periods. The credit-expansion channel this creates and the monitoring gap it implies are developed in (VAL.A (VAL.B (VAL.B §A).)6) and (VAL (VAL.B (VAL.B §7.3)).)
...
|:---|:---|:---|:---|
| **(VAL.B §J): Deferred delta** | Understatement defers, not eliminates | Post-sale assessment recovers accumulated basis gap in a single calculation | (VAL §1), (VAL.B §5.3); (VAL.A (VAL.B §C.)1), (VAL.B §C.7) |
| **(VAL.B §K): Dilution compounds** | Must-transfer cost tracks the asset's growth | State acquires equity at declared price; appreciation runs at true rate | (VAL (VAL.B §5.)2); (VAL.A (VAL.B §A.4.)5), (VAL.B §C.5) |
| **(VAL.B §L): Route D defers to realisation** | Annual cash on illiquid assets recreates forced-realisation pressure | Inheritance auction establishes observable price; full gain taxed at realisation | (VAL (VAL.B §6.)1), (VAL.B §6.5) |
| **(VAL.B §M): Voluntary settlement** | Soft/hard resets give certainty, not avoidance | Liability paid in full at declared or market value; basis resets for all future calculations | (VAL (VAL.B §6.)4); (VAL.B §1.0) |
| **(VAL.B §N): Forecast exposure** | Honest declaration has no directional exposure to trajectory | Both understatement and overstatement penalise forecast error; honest declaration is agnostic | (VAL (VAL.B §7.)1); (VAL.A (VAL.B §A.5.)1), (VAL.B §C.9) |
