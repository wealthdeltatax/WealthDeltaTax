...
| 0.3 | 9 Aug 2026 | §4 retroactively subsectioned (§4.1–§4.3); §4.3–§4.8 added (automation and tax-base migration); keywords updated; ENV-4 added to Open Questions. |
| 0.4 | 14 Aug 2026 | Sanity-check corrections: §2 "no ceiling" characterisation of understater penalty corrected — penalty compounds and plateaus at high growth rates (rate function logistic ceiling) rather than having no ceiling, consistent with VAL.A settled finding; §6 BEHAV §7.4 cross-reference corrected to BEHAV §7.2 (provisional refund notification is addressed under the Reciprocity Principle); Appendix A stale instruction to update 0.0 register entry for OQ #9 removed — that update had already occurred. |
| 0.5 | 15 Aug 2026 | Update §5 and §6 with new 2nd order effects |
...
What follows is not a general equilibrium model and should not be read as one. Several sections establish that a channel exists and trace the direction of its effect; they do not produce estimates. Where the direction is itself uncertain — where competing effects move in opposite directions and which dominates is an empirical question — the paper says so and assigns the question to Phase One or to the macroeconomic modelling paper that Phase One data will eventually make possible. The Phase One measurement agenda implied by each section is collected in §9.
...
What can be said without a formal model is that the growth-feedback channel is a real mechanism, that the RATES figures should be understood as conservative along this dimension, and that the existence of the channel strengthens rather than weakens the case for Phase One. If the feedback loop is operating at meaningful scale, Phase One will generate data that makes the magnitude estimable. If it is not, Phase One will establish that too. The measurement design for the consumption multiplier channel is discussed in §9.
...
The Phase One measurement agenda — declaration equilibrium tracking, consumption multiplier estimation, capital velocity after Route C and D settlement, financial stability dampening observation, housing buyer composition analysis — is specified in §8 of this paper. The administrative-layer interventions in BEHAV generate the data records that make these measurements possible.
...
The inflationary rotation trigger threshold and accelerated holding period for Route C positions (§7, referred to GOV.B §E.7) are Governing Council calibration parameters specified in the Custodian's statutory mandate.
...
The demand composition shift (speculative demand falling, owner-occupier demand rising) is directionally described in §8 but the net price effect is ambiguous. Established in §9 as requiring Phase One data on buyer composition changes in markets with high WDT-eligible ownership concentration.
