...
The most dangerous form of the suppression strategy is partial imitation, for the reasons established in (FM (FM §2).) Opposition is visible and can be answered directly. A partial imitation is harder to identify, harder to oppose, and more damaging if it is implemented and attributed as a WDT failure.
...
This paper was completed in August 2026. No jurisdiction had implemented the Wealth Delta Tax at that date. The binary equilibrium described in (FM (FM §1)) had not yet resolved. The dynamics described throughout were analytical predictions, not observations.
...
The intellectual record serves two purposes. The predictions in (FM (FM §1)) are specific enough to be checked against events as they unfold: if the adoption curve produces the pattern described (long silence, then a race), this document is dated evidence that the pattern was anticipated before it occurred; if the partial imitation dynamic described in (FM (FM §2)) produces failed implementations attributed to the WDT, this document names that dynamic and its mechanism in advance.
