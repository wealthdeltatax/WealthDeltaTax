...
| 1.2      | 07 July 2026  | Cross-reference corrections: (GOV §5.3)/(CORP.A §4)/(CORP.A §6.2) → (GOV §6.3)/(CORP.A §5)/(CORP.A §7.2) (§C.4); Veto Holder removed; executive body count corrected to six |
| 1.4      | 21 July 2026  | Parameter framework revision: (CORP.A §B.1) rewritten to derive $\tau_0$ floor from collection-security logic rather than anti-arbitrage; (CORP.A §B.2) rewritten to derive $\tau_h$ range from competing objectives (deterrence, revenue recovery, refund forfeiture, legal proportionality) rather than asserting $\tau_m$ as natural anchor; (CORP.A §B.3) updated to reflect collection-security framing; (CORP.A §C.3) updated to reframe (RATES (CORP.A §8.)1) result, $\tau_m$ as ceiling rather than anchor|
| 1.5      | 29 July 2026  | Rate calibration further developed: (CORP.A §B.1) extended with expected-value calibration expression, structural analysis of failure population, and explicit Phase One data requirements; (CORP.A §B.2) substantially revised — self-defeating litigation argument added as discrete analytical observation (establishing standing is itself an attribution event, dissolving the proportionality constraint as a binding upper bound), refund-forfeiture effect quantified directionally, calibration range restated as [deterrence floor, $\tau_m$] with Governing Council discretion within it, Phase One data requirements stated; bridging note between (CORP.A §B.1) and (CORP.A §B.2) added; (CORP.A §B.3) updated to note self-defeating litigation applies at intermediary level too; (CORP.A §C.3) updated to remove legal proportionality as competing pressure and reflect revised (CORP.A §B.2) framework |
| 1.7      | 3 August 2026 | Formatter pass: (CORP.A §B.1) AI tell removed ("shape what that bound looks like in practice" replaced with "bear on this bound"); (CORP.A §B.2) temporal artifact removed ("In practice, the constraint is substantially weaker" framing); (CORP.A §B.2) AI tell reworded ("not a penalty with no exit: it is a default charge" inverted to "a default charge that dissolves on voluntary identification, not a penalty with no exit"); (CORP.A §B.2) temporal artifact removed from calibration range paragraph ("rather than a single value" qualifier) |
...
One finding belongs in the whitepaper rather than here. Voluntary below-threshold WDT participation addresses a retail shareholder below the exemption threshold who has nothing to reconcile against by default. The position: below-threshold individuals should be able to opt in voluntarily, with full symmetric treatment, at $\tau_0$, with no minimum-commitment period, since the whitepaper's lifetime contribution envelope (W(P (CORP.A (CORP.A §3)).)5) already prevents an entrant from joining ahead of an expected loss and exiting with an unearned refund. This finding belongs in (WP (CORP.A (CORP.A §3.4)).) This paper draws on it only for the narrow consequence to shareholder reconciliation eligibility: an opt-in retail shareholder becomes an ordinary native taxpayer under (CORP (CORP.A §5))'s existing reconciliation logic, with nothing new required of the corporate mechanism.

## C.2 The Valuation Paper

Private company equity is in scope of the WDT through individual assessment, not the corporate instrument. This paper confirms that division and assigns the design of that assessment to (VAL). The synthetic exposure and derivatives treatment in (CORP (CORP.A §7)) requires coordination on derivative valuation methodology for individual WDT purposes where no liquid secondary market exists; this remains open and is assigned to (VAL).
...
| Broker/custodian nominee | Usually yes | Pass-through to identified clients; $\tau_h$ on unattributable remainder | Standard case; see (CORP (CORP.A §5.)4) and (CORP.A §A.9) |
...
Where a settlor retains effective control, or where the beneficial class is narrow and identified, the attribution chain is clear. The trust passes through on the terms established in (CORP (CORP.A (CORP.A §5).)5), with each beneficiary's allocated delta flowing into their individual WDT assessment. Where the beneficial class is broad and discretionary, attribution may fail, and $\tau_m$ applies at the entity level as the default.
...
As noted in (CORP.A (CORP.A §F.8)), the conditions required to sustain a meaningful attack through structural means are progressively harder to satisfy as the mechanism matures. The WDT substantially closed this attack vector through its attribution architecture before the attack was identified as a problem.
