...
The SRR sizing is expressed in units of observed net annual WDT revenue, not in units of any predicted declaration ratio. The TCM computes revenue by applying the rate function to population-weighted deltas across all brackets and growth tiers; the 3× capitalisation target is then a multiple of that revenue figure. The declaration ratio α does not appear as an explicit input. The SRR's response to population-level declaration behaviour is therefore automatic and endogenous: if the population systematically overstates (α > 1), declared deltas are larger than true deltas, gross revenue is correspondingly higher, and the 3× target fills at a faster pace than the α = 1 baseline model implies. If the population systematically understates (α < 1), the reverse holds and fill is slower. The Governing Council does not need to estimate the population α in advance or build an explicit correction into the SRR target. The calibration self-adjusts to whatever declaration behaviour the system produces, and Phase One data on declared values relative to realisation prices (PHASE1 §5.4) will provide the first empirical basis for assessing whether the revenue stream reflects systematic deviation in either direction. The ENV §2 observation that a mild-overstatement equilibrium near α ≈ 1.1 would call for a modestly upward SRR calibration should be read as a directional note for the Governing Council's first post-Phase-One review, not a pre-launch sizing requirement.
