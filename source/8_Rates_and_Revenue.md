...
The burden figures in this section are used by (WFR) as the revenue-equivalence baseline for its six-system welfare comparison: WFR calibrates all systems to E[T] = 2% of $W_0$, which corresponds to the revenue-weighted average annual wealth burden of 0.35% of net worth established here. The tier-level wealth trajectories and growth differentials in (RATES §5.2) supply the four-tier Fagereng calibration that WFR uses in its concentration analysis (WFR §4.3).
...
Full symmetry of the loss-refund mechanism is a settled design position, not a parameter. Partial symmetry (refunding losses at a rate below the applicable marginal gain rate) fails on two distinct grounds. Mechanically, it breaks the @DomarMusgrave1944 risk-sharing logic: the government functions as a proportional partner in the taxpayer's risk only when it absorbs losses at the same rate it captures gains. On foundational grounds (MF §6); (MF §7), the symmetric refund is the operational expression of the state accepting downside exposure alongside the taxpayer; partial symmetry abandons that commitment rather than calibrating it. Formal numerical modeling is undertaken in (WFR).
...
The corporate rate-floor question is not settled by this paper (SCOPE §5). The full derivation is in CORP.A §B.1–B.2, which disaggregates the question into two distinct optimisation problems: $\tau_{prov}$ (an administrative cash-flow parameter) and $\tau_h$ (a final charge on permanently unattributable ownership). Under that framework, $\tau_m$ is the ceiling of the plausible range for $\tau_h$ rather than a predetermined anchor. The revenue model in (RATES §8.1) retains $\tau_c$ = $\tau_m$ = 70% as the working ceiling assumption for estimation purposes.
...
The SRR's target balance is not derived from the theoretical refund ceiling alone. That ceiling (the maximum liability if all taxpayers simultaneously claimed maximum refunds with no offsetting tax income) is astronomically large and would take decades to reach in any scenario. The operational sizing target is the SRR capitalisation ratio: the SRR target balance as a multiple of net annual WDT income under normal conditions. The recommended Governing Council floor is 3×. The 3× figure is chosen to be achievable within a single political cycle from launch, providing an early credibility milestone without requiring an implausibly long capitalisation phase. It is a Governing Council parameter under the Tier 1 process (GOV §6), not a fixed design constant.

The SRR sizing is expressed in units of observed net annual WDT revenue, not in units of any predicted declaration ratio. The TCM computes revenue by applying the rate function to population-weighted deltas across all brackets and growth tiers; the 3× capitalisation target is then a multiple of that revenue figure. The declaration ratio $\alpha$ does not appear as an explicit input. The SRR's response to population-level declaration behaviour is therefore automatic and endogenous: if the population systematically overstates ($\alpha$ > 1), declared deltas are larger than true deltas, gross revenue is correspondingly higher, and the 3× target fills at a faster pace than the $\alpha$ = 1 baseline model implies. If the population systematically understates ($\alpha$ < 1), the reverse holds and fill is slower. The Governing Council does not need to estimate the population $\alpha$ in advance or build an explicit correction into the SRR target. The calibration self-adjusts to whatever declaration behaviour the system produces, and Phase One data on declared values relative to realisation prices (PHASE1 §5.4) will provide the first empirical basis for assessing whether the revenue stream reflects systematic deviation in either direction. The ENV §2 observation that a mild-overstatement equilibrium near $\alpha$ $\approx$ 1.1 would call for a modestly upward SRR calibration should be read as a directional note for the Governing Council's first post-Phase-One review, not a pre-launch sizing requirement.
...
## 7.4 Route D Deferred Revenue and Capitalisation Window Timing

The TCM models all taxpayers as generating annual delta revenue in each assessment period. This is correct for Routes A, B, and C, where the declared delta produces an annual tax liability or refund. It is not correct for Route D, where all settlement defers to a realisation event (a sale, death, or inheritance trigger) and no annual revenue flows until that event occurs.

Route D revenue is not lost; it is deferred. A Route D taxpayer who holds a position for twenty years and realises at year twenty contributes nothing to SRR or LRR fill in years one through nineteen and a potentially large lump sum in year twenty. The relevant question is how much Route D revenue falls within the capitalisation window rather than after it.

Annual realisation flows from the Route D population are estimable from three existing sources without live WDT data: ONS mortality data cross-referenced with the upper-tail wealth distribution (deaths trigger inheritance settlement); HMRC transaction data on private company sales and commercial property disposals (the principal Route D asset classes); and the inheritance and estate settlement record. That exercise has not been conducted here; it is a pre-Phase-One analytical deliverable classified in (SCOPE §3.4).

Pending that estimate, the RATES fill timelines should be understood as upper bounds on capitalisation speed. The degree of overstatement is proportional to the share of Route D wealth that does not realise within the capitalisation window. To bound the exposure: if Route D accounts for 20% of WDT-taxable wealth at the upper tail (the top of BEHAV.A §A.5's estimated range) and no Route D position realised within the first thirty years of WDT operation, the revenue shortfall relative to the TCM baseline would be approximately 20% of the upper-tail contribution during that period, recovered in full at realisation thereafter. That scenario requires the simultaneous non-realisation of every Route D position across three decades, which is inconsistent with observed mortality rates and private asset transaction frequencies. The realistic impact is materially smaller and concentrated in the early capitalisation years, where the SRR's 3× net revenue target provides the relevant buffer.

The SRR's year-3 fill invariance result is robust to Route D deferrals by construction: the target is expressed as 3× observed net annual WDT revenue, so if deferrals reduce the annual revenue stream the target scales down proportionally and the fill timeline remains approximately three years regardless.

The LRR is different. The LRR floor is fixed in real terms (expressed as years of government expenditure, not as a multiple of observed revenue) and does not scale with the annual revenue stream. A sustained Route D deferral effect would extend the LRR breakeven year beyond the RATES §7.2 figures by a margin proportional to the revenue shortfall during the capitalisation window. Item #33 is designed to establish whether that extension is material. Given observed mortality rates and private asset transaction frequencies, the realistic effect is concentrated in the early capitalisation years and is unlikely to approach the theoretical upper bound of a 20% sustained revenue reduction across the full window.

## 7.4 Revenue Under the Predicted Declaration Distribution**

The TCM assumes $\alpha$ = 1 throughout: every taxpayer declares their assets at true value in every assessment period. The behavioural analysis in (VAL §7), (VAL.A §A.6), and (ENV §2) predicts a population centre modestly above honest declaration, driven by refund-protection asymmetry under valuation uncertainty. (SWEEPS §11) establishes that mechanism intensity varies with wealth level. No paper in the series computes revenue under a population-weighted declaration distribution. This section addresses that omission.

The revenue consequences of departure from $\alpha$ = 1 are asymmetric in a specific way. Systematic understatement ($\alpha$ < 1) produces higher revenue than the baseline, not lower. An understating taxpayer declares a basis below true value; the basis gap compounds against them as the asset grows; the deferred liability recovers in full at realisation at the applicable marginal rate, which is higher than the rate that would have applied to the smoothed annual delta. The state collects more from a systematic understater than from an honest declarer on the same asset.

Systematic overstatement ($\alpha$ > 1) also produces higher revenue during the accumulation period. A population centred at $\alpha$ = 1.1 declares a tax base modestly above true values; annual deltas are correspondingly larger; the state collects slightly more than the honest-declaration baseline in each gain year. The RATES pre-behavioural revenue figures are therefore conservative with respect to the declaration distribution in the same direction as every other modelling assumption: actual declared revenue under any plausible declaration distribution is modestly higher than the TCM baseline.

The one scenario in which the declaration distribution creates a genuine risk is coordinated mass refund timing. If the population systematically overstates long enough to accumulate large lifetime envelope balances, and then coordinates maximum refund claims to fall in the same assessment year, the SRR faces a transient liquidity demand materially above its normal sizing. This is the declaration distribution's worst case for the state: not a revenue shortfall, but a timing mismatch between accumulated refund entitlements and available SRR reserves.

Three structural features bound this risk. The lifetime contribution envelope caps each taxpayer's refund entitlement at their cumulative prior contributions, so the mechanism cannot return more than it has collected from any individual regardless of declared loss size. The SRR is sized against a realistic refund exposure derived from the historical return series, not against the theoretical maximum of simultaneous full-envelope drawdown across the entire population. And the coordination requirement is itself a significant barrier: achieving a population-level timing event requires thousands of taxpayers and their professional advisers to simultaneously engineer large declared losses in the same assessment year. The population most capable of attempting this is also the population whose behaviour is most visible to the Administrator through the route distribution and declared value records, and whose coordinated action would constitute the egregious outlier pattern the corrective Route D auction pathway and Tier 1 monitoring are designed to detect.

The $\alpha$ = 1 assumption in the TCM is conservative in both directions that matter for the revenue claims: systematic understatement and systematic overstatement both produce higher annual revenue than the baseline. The coordinated mass refund scenario is the only declaration-distribution outcome that creates a genuine SRR risk, and it is bounded by the envelope cap, the SRR's reserve sizing, and the coordination barriers inherent in a dispersed population. The net effect of relaxing the $\alpha$ = 1 assumption in the direction the behavioural analysis predicts is modestly higher revenue and a modestly faster LRR fill. A population-weighted revenue calculation across the predicted $\alpha$ distribution — using the existing model infrastructure and the RATES bracket populations — is a natural analytical extension classified in (SCOPE §3.4), and would confirm the direction of this argument with specific figures.
...
The additivity of the corporate levy estimate and the individual WDT figure of £874b follows directly from the table. The only population that could produce double-counting (native WDT shareholders whose listed equity is already included in their declared net worth) contributes zero net corporate levy revenue, because the provisional levy on that tranche is released in full on individual settlement. The £172b reference estimate derives entirely from beneficial owners who have no individual WDT assessment position, whose wealth is therefore not captured in the TCM's £874b figure, and whose market capitalisation delta is incremental revenue rather than a duplicate collection.
...
The formal welfare comparisons that this paper's burden figures implied but did not carry out — comparing the WDT against capital gains tax, income tax on capital, a stock wealth tax, a consumption tax, and an accrual tax at genuine revenue equivalence — have since been undertaken in (WFR §3) and  (WFR §4). The Domar-Musgrave extension to a progressive delta base, identified as a gap in (LR.A §2.1), is addressed in (WFR §3.2) and (WFR §4.1). The distributional concentration arithmetic across a four-tier Fagereng calibration, identified as a gap in (LR.A §2.3), is in (WFR §4.3).
...
No items in this paper beyond those inherited from CORP — specifically the corporate instrument transition and CIT displacement question (SCOPE §5) — and GOV (Governing Council milestone declaration criteria).
