...
| 2.1      | 20 August 2026  | §9 updated to incorporate privacy election as upstream mechanism determining register content (GOV.B §B.4.7); cross-reference updated from GOV §5.3 to GOV.B §B.4.6–B.4.7; register inflation consequence of mild-overstatement equilibrium added; disclaimers section unchanged |
...
Figure 7.3 shows the terminal net worth gap against holding period for both understaters and overstaters at the historical mean growth rate. Understater penalties (red) widen substantially as N increases, while overstater TW advantages (blue) are broadly stable across holding periods. Mild understaters ($\alpha$ = 0.8) show modest and relatively stable penalties throughout; severe understaters ($\alpha$ = 0.1, 0.2) show dramatic penalties at long N. The cost asymmetry is a time-compounding effect, not merely a growth-rate one.

![Figure 7.3: Terminal net worth gap relative to honest declaration, by holding period N, at historical mean growth rate $g$ = 10.5%. Red lines: understater strategies ($\alpha$ = 0.1 to 0.8). Blue lines: overstater strategies ($\alpha$ = 1.2 to 2.0). $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.8).](../figures/val_fig_04_c8_tw_gap_by_n.png){width=100%}
...
Figure 7.5 shows the reversal structure directly. The left panel plots C.1 by growth rate for four overstatement levels: $\alpha$ = 1.2 and $\alpha$ = 1.5 remain below zero across the full growth range tested; $\alpha$ = 1.8 and $\alpha$ = 2.0 peak in advantage at low growth, cross into disadvantage at approximately $g$ = 9–10%, then recover advantage above approximately $g$ = 16–17%. The right panel shows these reversal and re-reversal thresholds as bars. The absent columns for the mild overstaters capture the forecast-independence claim in a single image.

![Figure 7.5: Overstatement reversal thresholds. Left panel: C.1 metric by growth rate $g$ for four overstatement levels ($\alpha$ = 1.2, 1.5, 1.8, 2.0). C.1 < 0 = advantage over honest declaration; C.1 > 0 = disadvantage. Right panel: first-reversal and re-reversal growth thresholds for each $\alpha$ level; absent bar = no reversal across tested range. N = 29, $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.1), (VAL.A §A.6).](../figures/val_fig_06_overstatement_reversal_boundary.png){width=100%}
...
The Python model (v1.0) is validated with confirmed figures at unified parameters. Directional claims stand: understatement is more expensive than honest declaration across the policy-relevant growth range, and aggressive overstatement provides no reliable advantage at moderate growth. The stable population equilibrium is mild overstatement ($\alpha$ ≈ 1.2–1.5), not exact honest declaration — a distinction that matters for refund exposure but not for the revenue claim. The state's objective is to collect the tax owed on actual wealth accumulation, not to enforce declaration precision. (VAL.A §C.1) confirms a wide region around honest declaration within which revenue collected is negligibly different from what exact honesty would produce; the (SWEEPS) develops this point formally. Three empirical questions remain open.
