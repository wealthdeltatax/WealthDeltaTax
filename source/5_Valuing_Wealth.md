...
(VAL.A §A) sets out the mathematical model. (VAL.A §C) contains simulation results across declaration strategies and growth scenarios, calibrated to the Balanced transition scenario in (RATES). At RATES-aligned parameters, understatement is more expensive than honest declaration across all moderate growth rates tested, and the refund reduction from understatement is substantial. Mild overstatement ($\alpha$ ≈ 1.2–1.5) produces a nominal TW_settled advantage that does not survive discounting: (VAL.A §C.12) shows that once periodic outflows and the sell-year refund are discounted at 5%, the apparent advantage collapses or reverses throughout the growth range where it nominally exists. The genuine rationale for a mild-overstatement equilibrium is therefore not a return to overstatement but asymmetric refund protection under valuation uncertainty: a risk-averse taxpayer facing a ±10–20% estimation band around their true asset value rationally centres their declaration slightly above their central estimate — near α ≈ 1.1 — so that the lower end of their uncertainty band still lands in overstatement territory, preserving a larger refund entitlement in a loss year. Aggressive overstatement faces a growth-corridor requirement that makes it irrational for most taxpayers under most conditions. What the system does not enforce is precision, only that the consequences of a declared value accumulate over time in ways the taxpayer cannot easily escape.
...
![Figure 5.2: C.1 metric heatmap: lifetime net tax difference as a share of terminal wealth, across declaration ratio $\alpha$ (rows) and annual growth rate $g$ (columns). Red = taxpayer pays more than honest declaration; blue = taxpayer pays less. N = 29, $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.1).](../figures/val_fig_02_c1_tax_difference_heatmap.png){width=100%}

Figure 5.3 decomposes the nominal TW_settled figures into their three mechanical components. The left panel shows that the sell-year refund benefit (blue) exceeds the f_N erosion cost (salmon) and post-sale damping cost (orange) across all tested $\alpha$, with the net nominal TW_settled figure (black line) rising to +11.5pp at $\alpha$ = 2.0. This decomposition explains the mechanism but not its economic significance: as (VAL.A §C.12) and Figure 10 establish, the nominal advantage does not survive discounting because periodic outflows precede the sell-year refund by the full holding period. The right panel maps f_N($\alpha$) / f_N(1) across the ($\alpha$, $g$) surface: darker shading indicates more equity eroded relative to the honest declarer, with the 0.95 and 0.90 contours showing where dilution becomes material.

![Figure 5.3: Overstater TW_settled advantage: mechanism decomposition (left) and equity dilution surface (right). Left: sell-year refund benefit swamps f_N erosion and post-sale damping costs across all tested $\alpha$ at $g$ = 10.4%, N = 29. The identity tw_adv = W_sell_delta − refund_delta − settle_delta is verified to machine precision; excess periodic tax is informational only and not additive. Right: f_N($\alpha$) / f_N(1) across ($\alpha$, $g$) space; darker = more equity eroded versus honest declarer; contours at 0.95 and 0.90. $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.11).](../figures/val_fig_08_tw_decomposition.png){width=100%}
...
Each declaration strategy has a payoff profile: a range of future outcomes under which it produces a better result than honest declaration. The understater's profile is profitable if growth is low; the cost accumulates through the basis gap recovered at realisation and the dilution mechanism on Route C if growth is moderate or high. The overstater's position is more complex once discounting is accounted for. On a nominal TW_settled basis, mild overstatement retains more terminal wealth than honest declaration across the tested range — but (VAL.A §C.12) establishes that this advantage does not survive discounting: the periodic outflows are real early money while the sell-year refund is inflated late money, and the time-value adjustment eliminates or reverses the advantage in the growth band where it nominally exists. Aggressive overstatement additionally faces a contemporaneous cost in the moderate-growth corridor where the bracket penalty dominates; that cost is independent of discounting. Honest declaration has no such conditional structure. Its outcome tracks the asset's actual trajectory without a second position on whether that trajectory falls within a particular range.
...
![Figure 10: Nominal (C.1) versus NPV-adjusted (C.12) tax difference relative to honest declaration. Left: C.1 metric as in Figure 7.1. Right: C.12 — the same metric with all tax cash flows discounted to t=0 at ρ = 5%. Shared colour scale. Low-g blue cells (mild-overstater nominal advantage) compress toward white or reverse to red in C.12 — the advantage does not survive discounting. Understater red cells are stable across both panels. N = 29, $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.12).](../figures/val_fig_10_c1_vs_c12_nominal_vs_npv.png){width=100%}
...
Figure 7.4 shows the TW_settled gap against holding period for both understaters and overstaters at the historical mean growth rate. Understater penalties (red) widen substantially as N increases, while overstater TW_settled advantages (blue) are broadly stable across holding periods. Mild understaters ($\alpha$ = 0.8) show modest and relatively stable penalties throughout; severe understaters ($\alpha$ = 0.1, 0.2) show dramatic penalties at long N. The cost asymmetry is a time-compounding effect, not merely a growth-rate one.

![Figure 7.4: TW_settled gap relative to honest declaration, by holding period N, at historical mean growth rate $g$ = 10.5%. Red lines: understater strategies ($\alpha$ = 0.1 to 0.8). Blue lines: overstater strategies ($\alpha$ = 1.2 to 2.0). $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.8).](../figures/val_fig_04_c8_tw_gap_by_n.png){width=100%}

Figure 7.5 shows the plateau structure directly. The excess tax burden for each understater strategy rises steeply through the $g$ = 10–17% range, then flattens at a ceiling proportional to the degree of understatement: near 99% of terminal wealth for $\alpha$ = 0.1, near 24% for $\alpha$ = 0.5, near 6% for $\alpha$ = 0.8. The penalty does not reverse above the plateau; the rate function simply stops escalating.

![Figure 7.5: Understater excess tax burden as a share of terminal wealth, by annual growth rate $g$. Each line represents one declaration ratio $\alpha$. The inflection point at $g$ ≈ 17% is where the rate function ceiling takes effect; ceilings are proportional to the degree of understatement. N = 29, $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.9).](../figures/val_fig_05_saturation_reversal_boundary.png){width=100%}
...
The outcome depends substantially on degree. On a nominal TW_settled basis, mild overstatement ($\alpha$ ≤ 1.5 at canonical parameters) retains more terminal wealth than honest declaration across the full tested growth range — the inflated basis generates a sell-year refund that nominally exceeds the f_N erosion and post-sale damping costs (VAL.A §C.11). However, this nominal advantage does not represent a genuine economic return: (VAL.A §C.12) shows that once all cash flows are discounted at ρ = 5%, the advantage collapses or reverses in the low-growth band where it nominally exists. At moderate and high growth, mild overstaters already pay more in net tax terms (C.1 > 0 from $g$ ≈ 8–10% onward for $\alpha$ = 1.2 and 1.5), and discounting makes the position worse still. The mild overstater's position is not forecast-free in any economically meaningful sense.

Aggressive overstatement ($\alpha$ ≥ 1.8) has an additional contemporaneous cost. The bracket penalty is large enough to produce a net cost relative to honest declaration throughout the $g$ ≈ 9–17% corridor — the range containing the historical mean and the majority of realistic trajectories. The aggressive overstater's net advantage recovers only outside that corridor, and the maximum nominal TW_settled benefit is bounded by degree: approximately 2.6pp at $\alpha$ = 1.2, 6.5pp at $\alpha$ = 1.5, 10.4pp at $\alpha$ = 1.8, and 13.0pp at $\alpha$ = 2.0 — each cap set by rate-function saturation rather than by holding period or growth (VAL.A §C.11d; Figure 7.3). All of these nominal figures are reduced or reversed under NPV adjustment.

A further refinement follows from valuation uncertainty rather than growth uncertainty. A taxpayer who does not know the precise value of their asset faces a declaration problem with a natural error band — typically ±10–20% for illiquid private assets. If they declare at their central estimate (α = 1.0 by assumption), their realised position could land anywhere within that band. The cost function around honest declaration is not symmetric: in the TW dimension, understating by 10% (α = 0.9) and overstating by 10% (α = 1.1) produce approximately equal and opposite TW differences at moderate growth, but the downside tails are not equivalent. An understater who is wrong in a bad year receives a materially smaller refund (VAL.A §C.6); an overstater in the same bad year receives a larger one. A risk-averse taxpayer aware of this asymmetry would rationally centre their declaration slightly above their central estimate — at approximately α = 1.1 rather than α = 1.0 — so that the negative end of their uncertainty band still lands in overstatement territory. This is a narrower and more precisely motivated version of the §A.6 mild-overstatement equilibrium, driven not by the TW_settled advantage of overstatement (which, as Figure 10 shows, does not survive discounting) but by the asymmetric refund protection it preserves. The mechanism accepts this: a population centred at α ≈ 1.1 rather than exactly 1.0 is one where every taxpayer's uncertainty band overlaps the tolerant zone, the revenue consequence is negligible, and the system's deterrence properties at the tails are unchanged.

Overstatement is therefore not a failure mode the mechanism needs to prohibit — it is self-regulating. Mild overstatement near the centre of a taxpayer's valuation uncertainty band is rational on refund-protection grounds (the §7.3 risk-aversion argument), and the mechanism tolerates it. It is not rational as a strategy for increasing terminal wealth: no overstatement level produces a genuine economic return once discounting is applied. Aggressive overstatement is self-limiting through the growth-corridor mechanism: the taxpayer who pushes too far takes on forecast exposure for a bounded and diminishing nominal upside that disappears under NPV adjustment. The quantitative boundaries are in (VAL.A §C.1), (VAL.A §C.12), and (VAL.A §A.6).

Figure 7.6 shows the C.1 net-tax structure directly. The left panel plots C.1 by growth rate for four overstatement levels: $\alpha$ = 1.2 and $\alpha$ = 1.5 remain below zero across the full growth range tested (mild overstaters pay less net tax in nominal terms), while $\alpha$ = 1.8 and $\alpha$ = 2.0 cross into positive C.1 (net-tax disadvantage) at approximately $g$ = 9–10%, then partially recover above $g$ = 16–17%. The right panel shows these reversal thresholds as bars. Note that C.1 < 0 for mild overstaters reflects the nominal net-tax metric only; as (VAL.A §C.12) establishes, the NPV-adjusted version of the same metric is near-zero or positive once discounting is applied to the same cash flows.

![Figure 7.6: Overstatement reversal thresholds. Left panel: C.1 metric by growth rate $g$ for four overstatement levels ($\alpha$ = 1.2, 1.5, 1.8, 2.0). C.1 < 0 = advantage over honest declaration; C.1 > 0 = disadvantage. Right panel: first-reversal and re-reversal growth thresholds for each $\alpha$ level; absent bar = no reversal across tested range. N = 29, $V_0$ = £20m, $k$ = 0.001. Source: (VAL.A §C.1), (VAL.A §A.6).](../figures/val_fig_06_overstatement_reversal_boundary.png){width=100%}
...
The Python model (v1.0) is validated with confirmed figures at unified parameters. Directional claims stand: understatement is more expensive than honest declaration across the policy-relevant growth range, and aggressive overstatement provides no reliable advantage at moderate growth. The population equilibrium sits slightly above $\alpha$ = 1 — near $\alpha$ ≈ 1.1 on the refund-protection rationale established in (VAL.A §A.6) — not at exact honest declaration. This distinction matters for refund exposure but not for the revenue claim. The nominal TW_settled advantage associated with mild overstatement does not survive NPV adjustment (VAL.A §C.12); the equilibrium near $\alpha$ ≈ 1.1 is driven by asymmetric refund protection under valuation uncertainty, not by a wealth-maximising return to overstatement. The state's objective is to collect the tax owed on actual wealth accumulation, not to enforce declaration precision. (VAL.A §C.1) confirms a wide region around honest declaration within which revenue collected is negligibly different from what exact honesty would produce; the (SWEEPS) develops this point formally. Three empirical questions remain open.

First, whether the systematic mild overstatement prediction — that the population equilibrium sits slightly above $\alpha$ = 1 rather than at exact honest declaration — actually materialises, and at what magnitude, requires Phase One declaration data compared against independently assessed values on Routes A and B. Second, which assessment windows taxpayers actually elect, and in what distribution across routes, cannot be predicted in advance; Phase One election data is the primary input to any window-premium calibration. Third, the SRR floor calibration implication of the mild overstatement equilibrium (ENV §9.2) cannot be quantified before Phase One data establishes how far above $\alpha$ = 1 the population equilibrium sits.
...
The formal model and simulations in VAL.A support the structural claim, though its correct formulation is more precise than the deterrent framing alone suggests. (VAL.A §C.1) establishes that across the range of declaration ratios from approximately $\alpha$ = 0.8 to $\alpha$ = 1.5, the lifetime net tax difference relative to honest declaration is economically negligible at the historical mean growth rate — a wide tolerant zone where the question of valuation accuracy becomes largely moot. Meaningful costs accumulate only at the tails: severe understatement at moderate-to-high growth and aggressive overstatement at moderate growth. The mechanism does not enforce accuracy; it concentrates consequences at the extremes while leaving the centre deliberately forgiving. (VAL.A §C.11) further decomposes the nominal TW_settled figures for mild overstaters: the sell-year refund benefit from the inflated basis exceeds the equity dilution and post-sale damping costs by approximately 6:1 at canonical parameters, explaining why the nominal figures are positive throughout — but this decomposition describes a timing structure, not a genuine economic return. (VAL.A §C.12) applies a 5% discount rate to all tax cash flows and finds that this nominal TW_settled advantage does not survive discounting in the growth band where it nominally exists: the mild-overstater equilibrium is better understood as a refund-protection asymmetry under valuation uncertainty than as a genuine economic return to overstatement. (VAL.A §C.6) establishes separately that the refund reduction from understatement means the cost of a low declaration runs in both directions. Whether taxpayers respond to these incentives as the mechanism expects is a question for Phase One evidence.
