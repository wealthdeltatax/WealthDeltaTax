...
Operational detail of the Custodian's conduct (bond sizing, closure bond facility mechanics, drawdown conditions, stewardship statement format) is in (GOV.B §E).
...
The solvency floor is a hard threshold expressed as a ratio of the fund's liquid assets to its aggregate outstanding liabilities: refund obligations, active closure bond facility commitments (CLOSE §5), and operating costs. The specific ratio is a Phase One calibration parameter and a Governing Council decision, informed by the Norwegian Government Pension Fund Global and the Chilean Pension Reserve Fund precedents identified in (GOV §6.3) as the reference models for Phase One consultation.

What is not a Phase One question is the principle governing the floor's operation. When the solvency ratio falls below the floor, automatic consequences follow without requiring a discretionary review or a Governing Council vote. The specific consequences (restrictions on the Custodian's discretion, mandatory reporting obligations, temporary constraints on new closure bond facility commitments) are Governing Council decisions calibrated to the circumstances at Phase One. The consequences are pre-specified, automatic, and do not depend on any party judging whether the breach was serious enough to warrant a response. The floor is a number, not a judgment call; crossing it is a fact.

The solvency ratio is computed on the same annual cycle as the assessment day. Intra-year monitoring is an operational matter for the Custodian; the annual assessment-day figure is the constitutional reference point against which the floor is tested and published. Aggregate closure bond facility commitments outstanding at any point count toward the ratio on the same basis as refund liabilities. The Custodian may not treat closure bond commitments as off-balance-sheet for solvency purposes; they are prior claims on the fund in the same category as the refund obligation.
...
## E.3 The Closure Bond Facility: Bond Posting Sequence and Settlement

The full structural design of the closure bond facility — covering death, exit, and bankruptcy procedural variants, the symmetric bond structure, the three delta-direction cases, and the rationale for decoupling interim activity from final settlement — is in (CLOSE §5) and (GOV §6.3). This section covers the operational sequence for the exit variant and the implementation principles the Governing Council's bond-sizing calibration must satisfy across all three variants.

The operational sequence (exit variant). The taxpayer declares an intended exit date through the Administrator's system, initiating the bond calculation process. The Custodian calculates the expected direction and magnitude of the final delta based on the taxpayer's declared asset values and current assessment window position. Bonds are posted before the taxpayer's declared exit date; the minimum notice period the taxpayer must give is a Phase One calibration parameter.
...
Bond-sizing principles. The specific bond-sizing ratios are Governing Council parameters calibrated against Phase One closure data across all three variants. Two constraints: the initiating-party-side bond must be large enough that the Custodian's secured position is not materially exposed to enforcement risk if the party fails to settle; and the state-side bond must be large enough that the taxpayer, estate, or creditor pool has adequate security for their expected refund. A state-side bond calibrated to a fraction of the expected refund does not satisfy the reciprocal commitment the cooperative architecture requires.

The Custodian holds administrative execution of the closure bond facility as part of its mandate, including managing bond calculations across all three variants, coordinating with the Administrator on declaration and initiation processes, holding initiating-party bonds as secured collateral, issuing state-side bonds, and executing settlement when the assessment process concludes. This includes all three procedural variants — death, exit, and bankruptcy — as specified in (CLOSE §5). Documentation requirements for the death and bankruptcy variants are assigned to a future GOV.B revision per (CLOSE §4.1) and (CLOSE §4.4).

The SWF holds three distinct credit instruments addressing different liquidity mismatches. The closure bond facility (CLOSE §5) addresses the timing mismatch across death, exit, and bankruptcy closure events where Route D assets require auction and other assets or distributions need to proceed before that auction completes. The sovereign liquidity facility (VAL §13) addresses the mismatch between an individual taxpayer's annual WDT liability and their available cash, providing a secured credit line against declared asset value for taxpayers on any cash-settled route. The corporate equity settlement facility (GOV.B §H) addresses the mismatch between a listed company's assessment-date levy and its cash generation cycle. All three are held in separate SWF portfolios with distinct mandate specifications and separate reporting lines in the annual stewardship statement.
