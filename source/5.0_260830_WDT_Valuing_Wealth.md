...
A caveat applies to the TW_settled surface: periodic overpayments are made at the time of the periodic assessment, while the sell-year refund arrives later. The nominal TW_settled advantage shown here is almost certainly negative in present-value terms once early cash outflows are discounted against the late refund. The surface establishes the structural incentive; it does not establish that overstatement is rational on a discounted basis.
...
The formal model and simulations in VAL.A support the structural claim, though its correct formulation is more precise than the deterrent framing alone suggests. (VAL.A §C.1) establishes that across the range of declaration ratios from approximately $\alpha$ = 0.8 to $\alpha$ = 1.5, the lifetime net tax difference relative to honest declaration is economically negligible at the historical mean growth rate — a wide tolerant zone where the question of valuation accuracy becomes largely moot. Meaningful costs accumulate only at the tails: severe understatement at moderate-to-high growth and aggressive overstatement at moderate growth. The mechanism does not enforce accuracy; it concentrates consequences at the extremes while leaving the centre deliberately forgiving. (VAL.A §C.11) further decomposes the mild overstater's TW_settled advantage: the sell-year refund benefit from the inflated basis exceeds the equity dilution and post-sale damping costs by approximately 6:1 at canonical parameters, confirming the advantage is structurally stable rather than growth-path dependent. (VAL.A §C.6) establishes separately that the refund reduction from understatement means the cost of a low declaration runs in both directions. Whether taxpayers respond to these incentives as the mechanism expects is a question for Phase One evidence.
