...
Wealth taxation · accrual taxation · wealth valuation · self-assessment · asset valuation · illiquid assets · valuation incentives · tax avoidance · tax compliance · Harberger taxation · self-assessed valuation · declaration incentives · unrealised gains
...
Overstatement works differently, and the outcome depends on degree. All figures below use TW_settled — terminal net worth after the post-sale tax or refund oscillation, the correct measure of lifetime outcome once the sell-year basis reconciliation is included. Mild overstatement ($\alpha$ ≤ 1.5 at canonical parameters) produces a stable TW_settled advantage across the full tested growth range. The mechanism is asymmetric: the inflated basis generates a proportionally larger sell-year refund entitlement that structurally exceeds both the f_N erosion cost (equity diluted faster by higher periodic tax) and the post-sale damping cost (oscillation taxes back part of the refund), by approximately 6:1 at canonical parameters (VAL.A §C.11). Rate bracket effects are real but secondary; the refund channel dominates. Aggressive overstatement ($\alpha$ ≥ 1.8) faces a different structure: the bracket penalty dominates in the $g$ ≈ 9–17% corridor containing the historical mean, and the TW_settled advantage is bounded near 13% at $\alpha$ = 2.0 regardless of how far the declaration is pushed, because the rate function saturates. The full matrix is in (VAL.A §C.9) and (VAL.A §C.5).
...
Figure 5.3 decomposes the mild overstater's TW_settled advantage into its three components. The left panel shows that the sell-year refund benefit (blue) exceeds the f_N erosion cost (salmon) and post-sale damping cost (orange) across all tested $\alpha$, with the net TW_settled advantage (black line) rising steadily to +11.5pp at $\alpha$ = 2.0. The right panel maps f_N($\alpha$) / f_N(1) across the ($\alpha$, $g$) surface: darker shading indicates more equity eroded relative to the honest declarer, with the 0.95 and 0.90 contours showing where dilution becomes material. At the historical mean growth rate (dashed line), the 0.95 contour is not reached until $\alpha$ ≈ 1.6, confirming that dilution costs within the mild-overstatement zone are modest relative to the refund benefit.
...
Figure 7.3 maps the TW_settled advantage across the full ($g$, N) surface for all four tested $\alpha$ levels. Each panel shows that the advantage is positive everywhere — overstatement always retains more nominal TW_settled than honest declaration across the tested range — but its magnitude and location differ by degree. For $\alpha$ = 1.2, the advantage peaks at 2.6pp at approximately $g$ = 0.6%, N = 34; for $\alpha$ = 1.5, at 6.5pp near $g$ = 1.1%, N = 41. These peaks occur at low growth and long horizons because the refund benefit from the inflated basis dominates when appreciation is slow and the holding period is extended. At the canonical intersection ($g$ = 10.4%, N = 29, red dot), the advantage is stable: 2.4pp for $\alpha$ = 1.2 and 5.3pp for $\alpha$ = 1.5. Aggressive overstatement panels ($\alpha$ = 1.8, 2.0) show the same pattern but with a low-growth, long-horizon peak and a compressed advantage near the canonical point, consistent with the growth-corridor and N-crossing results in (VAL.A §A.5.4).
...
The outcome depends substantially on degree. Mild overstatement ($\alpha$ ≤ 1.5 at canonical parameters) produces a TW_settled advantage that is stable and forecast-independent: the inflated basis generates a sell-year refund entitlement that structurally exceeds the dilution and post-sale damping costs at any realistic growth rate, so the advantage holds without requiring a correct trajectory forecast (VAL.A §C.11). If growth is slow, refund protection dominates; if growth is fast, the basis benefit at realisation dominates. Neither outcome requires the taxpayer to be right about which will occur.

Aggressive overstatement ($\alpha$ ≥ 1.8) has a different structure. The bracket penalty is large enough to produce a net cost relative to honest declaration throughout the $g$ ≈ 9–17% corridor — the range containing the historical mean and the majority of realistic trajectories. The aggressive overstater's net advantage recovers only outside that corridor, and the maximum TW_settled benefit is bounded by degree: approximately 2.6pp at $\alpha$ = 1.2, 6.5pp at $\alpha$ = 1.5, 10.4pp at $\alpha$ = 1.8, and 13.0pp at $\alpha$ = 2.0 — each cap set by rate-function saturation rather than by holding period or growth (VAL.A §C.11d; Figure 7.3). A taxpayer choosing aggressive overstatement bets their asset avoids the central distribution; that is a different kind of risk from the mild overstater's forecast-free position.
...
Figure 7.6 shows the reversal structure directly. The left panel plots C.1 by growth rate for four overstatement levels: $\alpha$ = 1.2 and $\alpha$ = 1.5 remain below zero across the full growth range tested; $\alpha$ = 1.8 and $\alpha$ = 2.0 peak in advantage at low growth, cross into disadvantage at approximately $g$ = 9–10%, then recover advantage above approximately $g$ = 16–17%. The right panel shows these reversal and re-reversal thresholds as bars. The absent columns for the mild overstaters capture the forecast-independence claim in a single image.
