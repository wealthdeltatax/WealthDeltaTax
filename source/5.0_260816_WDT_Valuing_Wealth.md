...
Each declaration strategy has a payoff profile: a range of future outcomes under which it produces a better result than honest declaration. The understater's profile is profitable if growth is low; the cost accumulates through the basis gap recovered at realisation and the dilution mechanism on Route C if growth is moderate or high. TThe overstater's advantage is not defined by a simple growth range: it depends on the interaction of declaration magnitude, growth, holding period, and the rate function. Moderate overstatement may remain advantageous, while aggressive overstatement becomes self-limiting over sufficiently long horizons; the timing and existence of that reversal are controlled principally by the entry rate rather than by progressive saturation alone. Honest declaration has no such conditional structure. Its outcome tracks the asset's actual trajectory without a second position on whether that trajectory falls within a particular range.
...
The Route D auction is not a single mechanism with a single trigger. It is a shared auction process that fires in three distinct circumstances, each with its own initiation architecture.

**Voluntary hard basis reset** (VAL §6.4): the taxpayer initiates the auction during the holding period to crystallise a market-verified basis before a forced realisation event. No Valuation Body involvement required. The purpose is certainty for the taxpayer, not enforcement.

**Inheritance trigger** (VAL §6.5): the auction fires automatically when a Route D asset passes to an heir. No taxpayer election and no Valuation Body consensus required; the transfer event is the trigger. The purpose is to close the Route D deferral at generational transfer and establish a market basis for the heir.

**Enforced trigger** (VAL §11.3): the auction fires following confirmation by two independent Valuation Bodies that the declared value is an egregious outlier. The taxpayer does not initiate. The purpose is deterrence of severe understatement and correction of accumulated basis gaps.

All three uses share the same auction mechanics: the asset is offered at the most recent recognised declared value as the opening price, any willing buyer may bid, the taxpayer or estate holds the right to retain at the highest third-party bid price, and the winning bid becomes the new recognised basis. What differs is who triggers the process.

## 11.1 Purpose and Deterrent Logic

In its enforced form, the Route D auction is the deterrent of last resort for egregious misvaluation of self-declared, non-fungible assets.

The mechanism applies in this form only to Route D. Routes A and B are professionally valued; mispricing liability rests with the valuator. Route C settles in kind through the must-transfer rule, creating continuous self-correction through dilution. Route D has no analogous interim correction. Between entry declaration and realisation, an understated basis compounds forward without any automatic adjustment. An egregious entry understatement permanently reduces the delta base. The enforced auction closes this gap where Valuation Body consensus confirms it exists.

The logic draws on the @Harberger1965 self-assessment tradition. A taxpayer declaring a private company at £5 million when it is worth £80 million faces losing it at the price they asserted was fair, through competitive bidding at that declared price. The cost of the misstatement falls on the person who made it. The state never needs to assert a correct value; the market supplies it. The deterrent does not require frequent triggering; its credibility rests on the near-certainty that a confirmed below-market declaration will attract genuine bids.

## 11.2 Relationship to Prior Self-Assessment Proposals

The WDT's auction mechanism draws on the Harberger deterrence logic while serving a different purpose from prior elaborations of it. The Posner-Weyl Common Ownership Self-Assessed Tax makes the purchase right universally available to any willing buyer, forcing assets to their highest-value use through continuous market pressure. The WDT's mechanism fires in the enforced case only on confirmed outlier status established through three-body valuation consensus; the purpose is reporting accuracy through deterrence, not allocative efficiency through forced transfer. The full comparison is in (LR.B §9).

## 11.3 Trigger Conditions and the Auction Process

Trigger conditions for the enforced case are strict. The full architecture — the three-body Valuation Body consensus mechanism, the sealed-estimate procedure that prevents anchoring, and the SWF Custodian's role in administering the auction — is specified in (GOV.B §G). In summary: a Valuation Body finding a Route D declared value to be a statistical outlier raises a flag and is then excluded from what follows; the other two produce sealed estimates opened simultaneously; the auction triggers only on their unanimous agreement that the gap exceeds the threshold.

When the trigger fires, the Administrator publishes notice of the auction. The asset is offered at the taxpayer's declared value as the opening price. Once a winning bid has emerged, the taxpayer may accept this bid as the new declared basis or sell to the highest bidder for cash. Cash proceeds increase net worth in the auction year, generating a positive delta taxed through the individual WDT assessment in the normal way. In either case the winning bid replaces the original declared value as the new recognised basis. A taxpayer who understated for an extended period does not escape the accumulated liability: the gap between the original declared value and the new basis is a large positive delta in the auction year, taxed at the applicable marginal rate. Administrative fees are charged to the taxpayer whose undervaluation triggered the process.

## 11.4 Scope

Rarity is a structural feature of the enforced trigger specifically. A mechanism triggering routinely would shift the institutional relationship toward the adversarial end and undermine the cooperative logic the WDT depends on. The deterrent effect rests on credibility, not frequency. The voluntary and inheritance uses are different: the voluntary hard basis reset is available on demand and expected to be used regularly by taxpayers wanting a verified basis before death, a financing event, or a dispute; the inheritance trigger fires automatically at every generational transfer of a Route D asset.

The full governance design for the enforced trigger — the non-discretionary trigger architecture, the three-body Valuation Body structure, Clause 8 of the enumerated structural list, and what remains open for Phase One calibration — is in (GOV.B §G).
