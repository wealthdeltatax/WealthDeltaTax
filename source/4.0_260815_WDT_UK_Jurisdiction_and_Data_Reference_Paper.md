...
**Rate function parameters (`tau_0`, `tau_m`, `k`, `$W_{min}$`).** Design parameters, not empirically derived quantities. The canonical values ($\tau_0$ = 15%, $tau_m$ = 70%, $k$ = 0.001 per £m, $$W_{min}$$ = £2m) constitute the Balanced scenario used throughout (RATES). Justification and full sensitivity analysis across the parameter space are in (SWEEPS) and (SWEEPS.A).
