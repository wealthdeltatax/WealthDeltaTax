...
Where a shareholder's annual WDT settlement and credit claim for a given year both fall in the same filing cycle as the corporate assessment date, a true-up in the following year may be required rather than same-cycle reconciliation. The mechanics of that true-up are a filing infrastructure question, not specified here.

The seller's right to initiate a partial-year correction under (CORP.A §A.3) follows the same timing. A seller who disposed of shares before the assessment date has one year from that assessment date to disclose the transaction and claim proportional attribution — the same window that governs credit claims and individual WDT settlement generally. In the limiting case, a seller who disposed one day before the assessment date retains one year and one day to decide whether to pursue correction. The Administrator may wish to impose a minimum claim size or flat processing fee to deter corrections whose administrative cost exceeds the credit recovered; this is a calibration parameter for the Governing Council.
...
Shareholder B sells in month four, before the assessment date. The gain enters B's individual WDT delta. B does not appear on the register at the assessment date. The corporation pays provisional levy on the full delta including B's tranche. B settles their individual WDT for this year and registers their credit claim within the standard reconciliation window. The tax authority matches B's corporate gain component to the company and releases the credit against B's liability. 

Shareholder C, who bought B's shares, is on the register at the assessment date, receives a delta statement for the full remaining year, and claims their credit within their own reconciliation window. The partial-year gap described in (CORP.A §A.3) is visible here: C's delta statement covers the company's full-year appreciation, not just the period from when C held the shares. B, who receives no corporate credit for their four-month holding period, may initiate a voluntary correction within the standard one-year reconciliation window from the assessment date; if B does so, C's credit is reduced to the eight-month share and B's four-month entitlement is restored.
...
**Securities lending borrower.** The lender's economic position during the loan period is a contractual claim to the return of equivalent shares plus any agreed interest or manufactured payments. At each assessment date, the lender's net worth includes the marked value of that claim — which tracks the underlying asset's value — plus accrued interest receivable. The borrower's net worth includes the shares at market value as a positive entry and the repayment obligation, net of accrued interest payable, as a negative one. The borrower's delta therefore reflects changes in the asset's value offset by the carrying cost of the loan; the lender's delta reflects the same asset value movement through the contractual claim, plus the interest accrual. Neither party loses economic exposure to the underlying asset during the lending period; the mechanism tracks both correctly through the net worth calculation without any special rule.
...
The corporate delta levy in a mature WDT replaces this with: one delta calculation, one provisional payment, one batch of delta statements, and one settlement account reconciliation at the year close. The tranche classification exercise is added, but it sits on existing infrastructure and is largely standing rather than annual. For a large listed company with a predominantly identifiable shareholder base, total compliance cost in a mature WDT is substantially lower than current CIT compliance. For large listed companies, the total burden falls.
...
## E.10 Public and Quasi-Public Entities

### E.10.1 The operative test

The attribution test applied throughout this section asks whether a continuous chain exists from a registered holder to an identifiable human beneficial owner whose individual WDT assessment can absorb the relevant delta. For private entities, failure of this test is a choice: the beneficial owner exists but has not been surfaced. The tax consequence, $\tau_h$, is calibrated to make that choice irrational.

Public and quasi-public entities present a different situation, but the distinction is not primarily one of attribution. It is one of base. The WDT taxes the change in individual net worth. The corporate levy collects on individual wealth that cannot be attributed at scale through listed-company ownership. Both instruments require a measurable base: a market capitalisation delta for the corporate levy, a declared net worth position for individual assessment. Where neither base exists, the mechanism has no purchase — not because the attribution test produces a zero rate, but because there is nothing for any rate to apply to.

A public entity that fails the attribution test and also generates no observable market capitalisation delta pays nothing under the corporate levy at any rate. $\tau_h$ applied to a zero base is zero. This is not a gap in coverage. It is the mechanism correctly identifying that no individual accumulation is occurring. Public entities do not appear in any individual's net worth because no individual owns them. The silence is the correct output.

The distinction between rate-silence and base-silence matters for the private gaming surface. An entity that fails the attribution test but has a private beneficial owner still has a Route D self-declared basis; the delta is measurable without a market price, and $\tau_h$ or the appropriate route-based charge applies. An entity with no private beneficial owner and no market price has no base at either instrument. Both conditions — attribution failure and base absence — must be present simultaneously for the mechanism to be genuinely silent. Attribution failure alone, in a privately owned entity, produces $\tau_h$ on the Route D delta. Base absence alone, in a publicly held listed entity with a market price, produces the normal corporate levy with $\tau_h$ on the unattributed tranche. Only when both are absent does the mechanism produce true silence.

The operative test for public entities is therefore sequential. First: is there a market capitalisation delta? If yes, the corporate levy runs through the normal tranche structure regardless of the entity's public character. Second: is there a private beneficial owner who can file a Route D self-declaration? If yes, individual assessment applies regardless of the entity's public character. Third: if neither condition is satisfied — no market price, no private owner — the mechanism is silent. The entity is outside scope not by exception but because both measurement instruments find nothing to measure.

A private entity cannot satisfy both conditions by instrument drafting. An entity with private beneficial ownership satisfies the second condition regardless of how its governing instruments describe the ownership relationship; the Route D self-declaration requirement attaches to the economic reality of ownership, not to what the instruments say about it. An entity with a market capitalisation satisfies the first condition regardless of how its revenues are sourced. The only genuine path to mechanism-silence is the absence of both a market price and a private beneficial owner — conditions that cannot be manufactured through document design because they reflect observable economic facts rather than legal characterisations.

The base migration surface requires a separate note. An individual transferring an appreciating asset from their own assessed position into an entity that will generate no future market delta triggers settlement at the point of transfer through the must-transfer mechanism or the appropriate route's realisation rules. The accumulated delta to the point of transfer is captured. Future appreciation inside the entity escapes individual assessment — correctly, where the entity is genuinely public with no individual beneficial owner; as a named residual limit, where the entity is structured to replicate public conditions while retaining private benefit. The mechanism closes the historical delta at transfer and accepts the residual on future appreciation as a boundary condition of the kind identified in (VAL §14.4) for Route D entry basis vulnerability. Both are named rather than papered over.

### E.10.2 Wholly public non-listed organisations

Public bodies holding assets for operational purposes without a market capitalisation generate no corporate delta. The mechanism is idle by normal operation, not by exception.

The National Health Service holds hospitals, clinical equipment, land, and capital infrastructure. None of this is expressed in a market price that moves on an assessment date. No annual delta arises. Staff receive salaries through their individual assessments in the normal way; salary is a fixed contractual payment, not a flow proportional to asset appreciation. When an NHS trust disposes of land or property, the proceeds flow into NHS accounts, not to any identified individual. The operative test returns no individual flow-through. The mechanism is silent throughout.

The military holds assets of substantial notional value — vessels, aircraft, weapons systems, land holdings, overseas infrastructure — whose operational value bears no relationship to any individual's enrichment. The same analysis applies. Defence contractors who build or service military assets receive contracted payments that enter their own corporate levy and individual assessments normally; the fact that the end-user is the Ministry of Defence is irrelevant to those chains. The MoD as asset holder generates no individual delta.

Where public bodies hold financial assets with observable market prices — sovereign gold reserves, foreign currency holdings, listed securities held through the Exchange Equalisation Account or similar vehicles — the operative test applies unchanged. No individual receives flows proportional to the appreciation of sovereign reserves. The mechanism remains silent regardless of the rate of appreciation: there is no base to which any rate applies.

Private finance initiative structures and outsourced public services are handled differently. Where a private company holds assets under contract serving public purposes, and that company's equity is held by identifiable shareholders above threshold, the corporate levy runs on the company normally. The public-purpose nature of the underlying activity does not affect the attribution chain.

Military defined-benefit pensions follow the treatment established in (CORP.A §C). The beneficiary's entitlement is a fixed promised payment, not a share of fund performance. Fund appreciation does not increase any individual's claim. The operative test correctly returns no individual flow-through.

### E.10.3 Government-held listed stakes

Where the state holds shares in a listed company — through a sovereign wealth vehicle, direct state ownership, or a government investment institution — the corporate levy runs at the company level normally. The government-held tranche is classified against the standard tranche structure: it is an identified holder that cannot produce an attribution chain to an individual beneficial owner. It therefore bears $\tau_h$ as a final charge on its share of the corporate delta.

This is the mechanism producing the correct result from its own logic, not an anomaly. The government-held tranche sits alongside private unattributed positions in tranche-two treatment, bearing the same cost for the same reason: no human beneficial owner has been identified. The incentive $\tau_h$ creates is identical: surface a human, and the charge ends.

For government-held listed stakes, surfacing a human means one of three things: selling into the market, transferring to a structure with identifiable private beneficial owners, or completing a full nationalisation that removes the entity from the listed threshold into the non-listed public organisation treatment above. $\tau_h$ therefore creates continuous pressure toward resolution of the ambiguous middle — the mixed ownership and partial state stakes that frequently reflect political inertia rather than principled design. Making inertia expensive is consistent with the mechanism's general posture.

The circularity concern — that the state is paying a charge to the SWF on assets it holds on behalf of the public — is less significant than it appears. The charge is proportional to the delta on the government tranche, which is proportional to the appreciation of the state's listed stake. A government that finds the charge burdensome has two mechanism-shaped responses: attribute the stake to identifiable private owners through sale or distribution, or accept the charge as the cost of maintaining a listed position without private attribution. The mechanism prices that choice; it does not make it.

### E.10.4 Quasi-public and mixed ownership entities

The space between clearly private and clearly public contains entities whose relationship between appreciation and individual net worth requires the operative test to be applied specifically rather than by category. Legal form does not determine treatment; the cash flow question does.

**Defined benefit pension funds** with government or employer backstop: the member's entitlement is a fixed promised payment. Fund appreciation improves the scheme's funding position; it does not increase any member's claim. The operative test returns no individual flow-through from appreciation. The mechanism is silent whether the backstop is a private employer or the state.

**Mutuals, housing associations, and similar member organisations**: appreciation flows to members only if and when distributed or realised. Where governing instruments prohibit distribution of appreciation to individuals — as is common in housing associations and many mutuals — the operative test returns no individual flow-through. Where governing instruments permit distribution, the mechanism captures the flow at the point it reaches an identified individual above threshold through their personal assessment.

**University endowments**: an endowment that accumulates appreciation without distributing it to identifiable individuals follows the accumulating endowment treatment in (CORP.A §E.2.3). Attribution typically fails by design; $\tau_h$ applies on net asset growth as a default charge, with voluntary beneficiary registration available to restore attribution where the endowment chooses it.

**Sovereign investment vehicles holding listed assets on behalf of the public**: the public does not receive flows proportional to the vehicle's asset appreciation; no individual converts sovereign investment returns into personal net worth above threshold through the vehicle's normal operation. The vehicle is constitutively collective by the test in (CORP.A E.10.1). The mechanism is silent on its listed holdings. Where the vehicle distributes returns to identifiable individuals — through a citizen dividend or equivalent — those distributions enter individual assessments at the point of receipt.

### E.10.5 Classified operational assets and the mechanism's outer boundary

A narrow class of state-held operational assets is held through structures specifically designed to obscure provenance as a matter of operational necessity. Front companies, nominee arrangements in third jurisdictions, and structures where filing would itself constitute a security breach present a case where both measurement instruments fail simultaneously: no market price, because the entity is unlisted and no third party transacts in its equity for operational security reasons; no filer, because identifying the beneficial owner would compromise the asset's function.

The operative test in (CORP.A §E.10.1) resolves this without a special rule. No market capitalisation delta arises. No private individual files a Route D self-declaration generating a personal delta. The mechanism is silent for the same structural reason it is silent on any entity satisfying both absence conditions — not because a decision has been made to exempt it, but because both measurement instruments find nothing to measure.

The economic substance confirms the silence is correct. Intelligence service operational assets generate no individual flow-through. Appreciation in a safe house, a technical installation, or a foreign property held under cover accrues to no identified person's net worth. The mechanism would return silence even if attribution information were fully available, because there is no individual delta to calculate. The opacity is operationally motivated; the fiscal result is the same as the NHS hospital case, reached through different means from the same structural position.

The $\tau_h$ question resolves the same way as the general public entity case. $\tau_h$ applies to unattributed positions bearing a delta. Where no delta exists, $\tau_h$ is irrelevant regardless of the rate. The classified asset case is not one where $\tau_h$ fires and produces zero because the attribution test fails on a zero base. It is one where both instruments find no base, and the question of what rate applies to the unattributed position does not arise.

The filing gap is accepted as a named residual limit. The mechanism cannot reach assets where the information architecture required for its operation is structurally incompatible with the asset's function. This is the outer boundary of the mechanism's reach, analogous to the Route D extreme residual in (MF §9.4.8): accepted because the entity it describes is at the margins of what the mechanism exists to address, because the economic substance test confirms the mechanism would be silent in any case, and because the revenue foregone is negligible.

The simultaneous failure of both measurement instruments on classified assets is not replicable by private entities through instrument design. A private entity structured to avoid a market price retains a private beneficial owner required to file a Route D self-declaration. A private entity structured to avoid a private beneficial owner — genuinely transferring ownership to a collective with no individual claim — loses the private benefit that motivated the structure. The classified asset case satisfies both absence conditions because the state holds the asset for operational public purposes with no individual enrichment occurring. A private individual cannot replicate this without either retaining beneficial ownership (which restores the Route D filing requirement) or genuinely relinquishing it (which removes the private benefit). The boundary condition is narrow and self-limiting.

Jurisdiction-specific legal analysis will determine how the WDT's enabling legislation addresses classified asset holdings in the reference jurisdiction. This is assigned to JUR.

### E.10.6 The emergent efficiency property

The WDT's silence on non-delta-generating public entities is not a designed feature of the public sector treatment. It is a consequence of the mechanism's general logic applied without modification.

Public organisations that hold assets for operational purposes, generate no appreciation-proportional flows to identifiable individuals, and are funded at levels consistent with their operational mandate bear no WDT exposure. Public organisations that accumulate assets beyond operational requirements, hold appreciating financial assets, or generate flows reaching identifiable individuals above threshold begin to generate WDT exposure proportional to those behaviours.

The fiscal gradient this creates tilts toward organisational forms that separate operational efficiency from individual capital accumulation. A structure that captures private energy without allowing appreciation to accumulate in individual hands — worker cooperatives distributing surplus as wages, mutuals providing member benefits rather than capital returns, social enterprises with locked appreciation — bears less WDT exposure than a private company of equivalent economic activity. Whether this produces beneficial organisational diversity or cost-discipline problems of the kind that characterise purely public delivery is an empirical question the mechanism does not resolve. The direction is clear; the magnitude is unknown and assigned to MACRO.

A public body receiving excess central funding sees its balance sheet improve; if it holds financial assets, a positive delta may arise. The following year, when funding normalises or assets are returned, the corresponding negative delta generates the symmetric refund. A public body consistently funded at its operational requirement generates no net delta over time and bears no WDT exposure. The mechanism is indifferent to organisational form; it observes only whether individual net worth has changed.
...
## F.1 $\tau_c$ as an Ownership-Composition Lever

$\tau_c$ sets the relative cost of three ownership states: individual WDT attribution, identified foreign beneficial ownership, and unattributed ownership. The Governing Council setting $\tau_c$ is not deciding only what foreign beneficial owners pay; it is deciding what composition of ownership the jurisdiction attracts.

The relevant comparison for a foreign wealth holder is $\tau_c$ against their effective individual WDT rate $\tau_{\mathrm{eff}}(W)$. Where $\tau_c$ exceeds $\tau_{\mathrm{eff}}(W)$, there is a rate incentive toward entering individual WDT assessment, through residence, domicile establishment, or other attribution events. Where $\tau_c$ approaches $\tau_{\mathrm{eff}}(W)$, the incentive weakens. For very high wealth levels, as $\tau_{\mathrm{eff}}(W)$ approaches $\tau_m$, the gap closes regardless of where $\tau_c$ sits.

A higher $\tau_c$ expands the population of foreign wealth holders for whom domestic attribution is economically preferable to remaining a foreign beneficial owner, and increases the cost of retained foreign ownership for those who do not migrate. But at sufficiently high levels it reduces the jurisdiction's attractiveness to large foreign holders who will not or cannot enter the individual system. The Council is choosing a target composition, not a revenue rate: $\tau_c$ is the price of identified-but-foreign ownership relative to the other two states.

A parallel effect operates on domestic TP members. Where $\tau_c$ exceeds $\tau_{\mathrm{eff}}$ for a domestic TP member, the differential creates an incentive to keep wealth attributable rather than allowing it to remain in a foreign corporate structure. A sufficiently large differential, however, creates incentives to engineer ownership arrangements specifically to preserve the lower individual rate, which reinforces the case for treating $\tau_c$ as a calibrated variable informed by Phase One ownership composition data rather than a fixed value.

The ownership-composition function is a consequence of the rate differential, not the primary calibration rationale for $\tau_c$, which is pricing the gap between attributed and unattributed foreign ownership on revenue-recovery grounds. Phase One data on foreign ownership share, domestic attribution rate trends, and investment volume by taxpayer type are explicit inputs to the $\tau_c$ review, alongside the revenue and attribution-rate data used to calibrate $\tau_0$ and $\tau_h$.

The mobility and substitutability assumptions implied by the composition-lever mechanism are Phase One empirical questions. Whether foreign wealth holders respond to $\tau_c$ by adjusting attribution status rather than absorbing the cost or reducing exposure is not established by the architecture alone; BEHAV does not model this population. The Governing Council should treat the composition effect as a property to be confirmed by Phase One evidence.
...
| $\tau_c$ | Charge on attributable foreign beneficial ownership: the rate borne by a foreign entity whose beneficial owner has been identified but is outside individual WDT reconciliation. Sits within $\tau_0 \leq \tau_c \leq \tau_h$. Its level determines the relative price of identified-foreign versus domestic-attributed ownership — the ownership-composition lever. (RATES) modelling used $\tau_c = \tau_m$ = 70% as a working ceiling assumption, which remains valid for revenue-estimation purposes pending Phase One data. | Governing Council calibration parameter; (CORP.A §F.1). (RATES.A) working assumption: $\tau_m$ = 70%. |
| $\tau_f$ | $\tau_f$ diplomatic rate-setting (bilateral/multilateral agreement; not a WDT parameter). Ceiling on unattributable ownership by foreign companies of WDT jusrisdiciton assets | Phase 1 Diplomatic calibration parameter |
